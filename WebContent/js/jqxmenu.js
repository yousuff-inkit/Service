    ...
    a.extend(a.jqx._jqxMenu.prototype, {
        ...
        minimize: function () {
            ...
            var b = function (k) {
                ...
                h.animate({ left: !j ? -h.outerWidth() : e.host.coord().left + e.host.width() + h.width(), opacity: 0 }, e.animationHideDuration, function () {
            ...
            var f = function (l) {
                if (e.minimizedHidden) {
                    ...
                    var j = h.width();
                    if (e.minimizedItem.css("float") == "right") {
                        h.css("left", e.host.coord().left + e.host.width() + j);
                        k = true;
                    }
            ...
            this.addHandler(a(window), "orientationchange.jqxmenu" + this.element.id, function (j) {
                setTimeout(function () {
                    if (!e.minimizedHidden) {
                        var k = h.width();
                        var l = false;
            ...
            this.addHandler(this.minimizedItem, "click", function (j) {
                f(j);
            });
        },
        restore: function () {
                ...
                var o = function () {
                    ...
                    if (b) {
                        k.stop().animate({ top: n }, m.animationHideDuration, function () {
                            ...
                            a(p.element).removeClass(m.toThemeProperty("jqx-rc-b-expanded"));
                            l.removeClass(m.toThemeProperty("jqx-rc-t-expanded"));
                            var q = a(p.arrow);
                            if (q.length > 0 && m.showTopLevelArrows) {
                                q.removeClass();
                                if (p.openVerticalDirection == "down") {
                                    q.addClass(m.toThemeProperty("jqx-menu-item-arrow-down"));
                                    q.addClass(m.toThemeProperty("jqx-icon-arrow-down"));
                                } else {
                                    q.addClass(m.toThemeProperty("jqx-menu-item-arrow-up"));
                                    q.addClass(m.toThemeProperty("jqx-icon-arrow-up"));
                                }
                            }
                            ...
                            if (p.level > 0) {
                                ...
                                if (q.length > 0) {
                                    ...
                                    if (p.openHorizontalDirection != "left") {
                                        ...
                                        q.addClass(m.toThemeProperty("jqx-icon-arrow-" + m._getDir("right")));
                                    } else {
                                        q.addClass(m.toThemeProperty("jqx-menu-item-arrow-" + m._getDir("left")));
                                        q.addClass(m.toThemeProperty("jqx-icon-arrow-" + m._getDir("left")));
                                    }
                                }
                            } else {
                                a(p.element).removeClass(m.toThemeProperty("jqx-fill-state-pressed"));
        ...
        _openItem: function (t, s, r) {
            ...
            var b = k
                ? j.left
                : this.menuElements[s.parentId] != null && this.menuElements[s.parentId].subMenuElement != null
                ? parseInt(a(a(this.menuElements[s.parentId].subMenuElement).closest("div.jqx-menu-popup")).outerWidth()) - f[0]
        ...
        _render: function (f, g) {
            ...
            a.each(this.items, function () {
                ...
                h.addHandler(o, "click", function (x) {
                    if (s.disabled) {
                        return;
                    }
                    if (h.disabled) {
                    ...
                    if (x.target.tagName != "A" && x.target.tagName != "a") {
                        var v = s.anchor != null ? a(s.anchor) : null;
                ...
                if (s.subMenuElement != null) {
                    ...
                    var r = a(s.subMenuElement)
                        ...
                        .css({ width: parseInt(a(s.subMenuElement).outerWidth()) + parseInt(d[0]) + "px", height: parseInt(a(s.subMenuElement).outerHeight()) + parseInt(d[1]) + "px" });
                    var t = o.closest("div.jqx-menu-popup");
                    if (t.length > 0) {
                        var l = p.css("margin-left");
                    ...
                    if (!h.clickToOpen) {
                        if (e || c) {
                            h.removeHandler(o, a.jqx.mobile.getTouchEventName("touchstart"));
                            h.addHandler(o, a.jqx.mobile.getTouchEventName("touchstart"), function (u) {
                                clearTimeout(p.data("timer").hide);
                                if (p != null) {
                                    p.stop();
                                }
                                if (s.level == 0 && !s.isOpen && h.mode != "popup") {
                                    u.data = h;
                                    h._closeAll(u);
                                }
                                if (!s.isOpen) {
                        ...
                        if (!c) {
                            h.addHandler(o, "mouseenter", function () {
                                if (h.autoOpen || (s.level > 0 && !h.autoOpen)) {
                                    clearTimeout(p.data("timer").hide);
                                }
                            ...
                            h.addHandler(o, "mousedown", function () {
                                if (!h.autoOpen && s.level == 0) {
                                    clearTimeout(p.data("timer").hide);
                                    if (p != null) {
                                        p.stop();
                                    }
                            ...
                            h.addHandler(o, "mouseleave", function (v) {
                                if (h.autoCloseOnMouseLeave) {
                                    ...
                                    var w = true;
                                    if (x.left - 5 <= u.left && u.left <= x.left + x.width + 5) {
                                        if (x.top <= u.top && u.top <= x.top + x.height) {
                                            w = false;
                                        }
                                    }
                                    if (w) {
                            ...
                            h.addHandler(r, "mouseenter", function () {
                                clearTimeout(p.data("timer").hide);
                            });
                            h.removeHandler(r, "mouseleave");
                            h.addHandler(r, "mouseleave", function (u) {
                                if (h.autoCloseOnMouseLeave) {
                                    clearTimeout(p.data("timer").hide);
                        ...
                        h.removeHandler(o, "mousedown");
                        h.addHandler(o, "mousedown", function (u) {
                            clearTimeout(p.data("timer").hide);
                            if (p != null) {
                                p.stop();
                            }
                            if (s.level == 0 && !s.isOpen) {
                                u.data = h;
                                h._closeAll(u);
                            }
                            if (!s.isOpen) {
        ...
        _createMenu: function (c, f) {
            ...
            for (var j = 0; j < t.length; j++) {
                ...
                if (g.disabled) {
                    r.addClass(v.toThemeProperty("jqx-menu-item-disabled"));
                    r.addClass(v.toThemeProperty("jqx-fill-state-disabled"));
                }
                if (f && !b) {
