...
<script type="text/javascript">
 ...
 $('#printWindow').jqxWindow({width: '51%', height: '28%',  maxHeight: '70%' ,maxWidth: '51%' , title: 'Print',position: { x: 300, y: 87 } , theme: 'energyblue', showCloseButton: true, keyboardCloseKey: 27});	
		 ...
		 $("#sqotprdname").change(function(){
				...
				var balqty = $("#sqotprdts option[value='" + $('#sqotprdname').val() + "']").attr('data-totalstock');
				//alert("allw=="+allw);
			...
			$("#sqotprdid").change(function(){
				...
				var balqty = $("#sqotproductid option[value='" + $('#sqotprdid').val() + "']").attr('data-totalstock');
				//alert("allw=="+allw);
	  ...
	  //alert(data);
	 	...
	 	function funwarningopen(){
			   ...
			   if (($("#mode").val() == "view") && $("#masterdoc_no").val()!="") {
			      var partsconfig=$("#partsconfig").val().trim();
			      if(partsconfig.trim()=='1'){
                       var filenametolad="";  //check popup windows for print
				       filenametolad="printVoucherWindow.jsp?partsconfig="+partsconfig;
		...
		window.open(reurl[0]+"printdeliveryNote?docno="+document.getElementById("masterdoc_no").value,"_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
				...
				var rowg=$('#rowindexg').val();
			     //alert("rowg1=========="+rowg);
			      if(document.getElementById("rowindexg").value!="" && !(document.getElementById("rowindexg").value==null) && !(document.getElementById("rowindexg").value=="undefiend")){
			    	  $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowg, "proid", document.getElementById("sqotprdid").value);
					     $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowg, "proname", document.getElementById("sqotprdname").value);
					     ...
					     $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowg, "productid" ,document.getElementById("sqotprdid").value);
					     $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowg, "productname", document.getElementById("sqotprdname").value);
...
//		 			     $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowlength-1, "collqty", document.getElementById("collqty").value);
			     ...
			     $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowlength-1, "proid", document.getElementById("sqotprdid").value);
			     $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowlength-1, "proname", document.getElementById("sqotprdname").value);
			     ...
			     $('#jqxDeliveryNote').jqxGrid('setcellvalue', rowlength-1, "productid" ,document.getElementById("sqotprdid").value);
			     $('#jqxDeliveryNote').jqxGrid('setcellvalue',  rowlength-1, "productname", document.getElementById("sqotprdname").value);
			...
			var pname=document.getElementById("jqxInput1").value;
			 ...
			 function funchkps()
				...
				function getProduct() {
						...
						if (x.readyState == 4 && x.status == 200) {
							...
							var prddatas=JSON.parse(items.trim());          
			  					...
			  					$.each(prddatas.pddata, function( index, value ) {
			  						optionsyearnw += '<option value="' + value.part_no + '" data-prdname="'+value.productname+'" data-unit="'+value.unit+'" data-brand="'+value.brandname+'" data-psrno="'+value.psrno+'" data-specid="'+value.specid+'" data-unitdoc="'+value.unitdocno+'"></option>';
			  						optionsyear += '<option value="' + value.productname + '" data-prdid="'+value.part_no+'" data-unit="'+value.unit+'" data-brand="'+value.brandname+'" data-psrno="'+value.psrno+'" data-specid="'+value.specid+'" data-unitdoc="'+value.unitdocno+'"></option>';
			  					});
					...
					x.open("GET", 'getProduct.jsp?prodsearchtype='+prodsearchtype+'&enqmasterdocno='+refmasterdocno+'&cmbprice='+cmbprice+'&clientid='+clientid+'&reftypes='+document.getElementById("cmbreftype").value+'&clientcaid='+clientcaid+'&dates='+dates+'&cmbbilltype='+cmbbilltype+'&location='+locationid, true);
					x.send();
				}
...
<body onload="checkReftype();setValues();getunit(0);getDefaultLocation(); getAlternativeConfig();getCurrencyIds();funchkps();getPrintExcelConfig();">	
  ...
  <tr>
    ...
    <td><select id="cmbreftype" name="cmbreftype" style="width:71%;" onchange="refChange();"  value='<s:property value="cmbreftype"/>'>
      <option class="refid" value="DIR">DIR</option>
       <option class="refid" value="SOR">SOR</option></select>
