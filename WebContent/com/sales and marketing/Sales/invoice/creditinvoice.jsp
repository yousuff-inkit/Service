...
function removeReadAttr(){

$('#txtclientdet').attr('readonly', false );
...
/*$('#frmSalesInvoice').validate({
	             rules: {
	             currate:{required:true,digits:true}
	             },
	             messages: {
	              currate:{required:" *",digits:"Digits only."},
	             }
	    });*/
	   ...
	   /* var specialKeys = new Array();
        specialKeys.push(8); //Backspace
         function IsNumeric(e) {
            ...
            var ret = (keyCode != 46 && keyCode > 31 && (keyCode > 48 && keyCode < 57) || specialKeys.indexOf(keyCode) != -1);
 ...
 */
 ...
 function chkstock()
	...
	function getUserSalesman(){ 
		...
		x.open("GET","getUserSalesman.jsp",true);
		x.send();
	}
...
// alert("Product counter : "+productcounter);
 ...
 var srows = $("#jqxserviceGrid").jqxGrid('getrows');
  ...
  newTextBox.val(srows[i].srno+"::"+srows[i].qty+" :: "+srows[i].description+" :: "
		   +srows[i].price+" :: "+srows[i].total+" :: "+srows[i].discount+" :: "+srows[i].nettotal+" :: "
 ...
 */				       
	...
	{
	 if(document.getElementById("cmbbilltype").value==""){
		 return false;
	 }
	...
	function getUnitPriceConfig(){
		...
		x.open("GET","getUnitPriceConfig.jsp",true);
		x.send();
	}
	...
	function calcincl(){
		...
		if(parseFloat(allw)>0 && parseFloat(disamt)>0){
			if(parseFloat(dis)<parseFloat(allw)){
...
<%-- <table width="100%" border="1">
 ...
 <table width="100%"  >
  <tr>                              
    <td width="8%"     align="right" onchange="getProduct()">Date</td>
...
<table width="100%"  >   
  ...
  <tr>
   ...
   <td  ><select name="cmbcurr" id="cmbcurr" style="width:73%;"  value='<s:property value="cmbcurr"/>' onload="getRatevalue(this.value);">
    ...
    <td colspan="3"><input type="text" id="currate"  name="currate" style="width:13%;" value='<s:property value="currate"/>'  onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" />
  ...
  <tr>
	...
	<td>
		<input type="text" id="salesman" name="salesman" style="width:70%;" value='<s:property value="salesman"/>' onkeydown="getSalesman(event);" placeholder="Press F3 to Search">
...
<td align="center">   <input type="text" id="dict"  onblur="funRoundAmt(this.value,this.id);" onkeypress="javascript:return isNumber1 (event);"   onchange="vatSwitch();"    style="text-align: right;"  ></td>
 ...
 <td align="center"> <input type="text" id="taxpers" onchange="vatSwitch();" tabindex="-1"      style="text-align: right;" ></td>  
...
<td><div style="position:fixed;z-index:1000;right:20px;top:15px;font-weight:bold;color:red;font-size: 12PX "> <label  >Order Value:</label> 
...
<input type="hidden" id="fixaccountname">     
      <input type="hidden" id="totalstock" name="totalstock"  value='<s:property value="totalstock"/>'/> <!-- balqtystock -->          
