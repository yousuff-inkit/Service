...
{	
	...
	descdata1='<%=purchaseorderDAO.gridFromExcel(session,excelDocNo,acno,dates,cmbbilltype)%>';  
}
else if(check>0){
	

	descdata1='<%=purchaseorderDAO.gridloadbasedonpop(session,"1",docno)%>';  
}
else
{   
	descdata1;
    ...
    $("#serviecGrid").on("bindingcomplete", function (event) {     			
                if($('#mode').val()=="A"){       		
	             ...
	             for(var i=0;i<rows.length;i++){
  	             	    ...
  	             	    var taxper= (typeof $('#serviecGrid').jqxGrid('getcellvalue', i, "taxper")==='undefined'?0:$('#serviecGrid').jqxGrid('getcellvalue', i, "taxper"));
            	  ...
            	  for(var i=0;i<rows.length-1;i++){
            		  ...
            		  var taxper= (typeof $('#serviecGrid').jqxGrid('getcellvalue', i, "taxper")==='undefined'?0:$('#serviecGrid').jqxGrid('getcellvalue', i, "taxper"));
	                	if(parseFloat(taxper)>0){
	                		  ...
	                		  $('#serviecGrid').jqxGrid('setcellvalue', i, "taxperamt",0);
	                		  $('#serviecGrid').jqxGrid('setcellvalue', i, "taxper",0);
	                		  $('#serviecGrid').jqxGrid('setcellvalue', i, "taxamount",netotal);
	                	}
