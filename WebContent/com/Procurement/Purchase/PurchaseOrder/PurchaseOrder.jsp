...
// For PurchaseMgmt Bi view
	...
	//System.out.println(brch+"=========brch value");
...
$(document).ready(function () { 
    ...
    $("#sqotprdname").change(function(){
		...
		var productid = $("#sqotprdts option[value='" + $('#sqotprdname').val() + "']").attr('data-productid');
		var proid = $("#sqotprdts option[value='" + $('#sqotprdname').val() + "']").attr('data-proid');
	...
	$("#sqotprdid").change(function(){
		...
		var productid =$("#sqotproductid option[value='" + $('#sqotprdid').val() + "']").attr('data-productid');
		var proid =$("#sqotproductid option[value='" + $('#sqotprdid').val() + "']").attr('data-proid');
	...
	$('#btnexcel').click(function(){
		$("#sevdesc").excelexportjs({
			...
			worksheetName:"Purchase Order Items"
		});
	});
  	 ...
  	 if(mod1=="A")  
		{
  		...
  		<%-- $("#sevdesc").load("serviecgrid.jsp?prcharray="+'<%=purchasearray.replaceAll("\\s","a@b@c")%>'+"&modebprf=a1"); --%>
		  	...
		  	$("#sevdesc").load("serviecgrid.jsp?tempdocno="+tempdocno+"&dates="+document.getElementById("nipurchaseorderdate").value+"&check="+check);
...
function popgridload(){
	if(check>0){
  		$("#sevdesc").load("serviecgrid.jsp?check="+check+"&docno="+tempdocno);
  	}
}
 ...
 function roundval()
{
 ...
 function funrefdisslno()
 {
	 ...
	 if(document.getElementById('reftype').value=="DIR")
		 ...
		 {
		 $("datalist#sqotproductid").html('');
		 $("datalist#sqotprdts").html('');
		  $('#rrefno').attr('disabled', false);
		  $('#rrefno').attr('readonly', true);
		  //$('#setbtn').attr('disabled', true );  
		  $('#det').attr('disabled', true ); 
		...
		function funRoundAmt4(value,id){
	     ...
	     function setgrid() {
	    	 //alert('on set grid')
	    	    ...
	    	    $('#serviecGrid').jqxGrid('setcellvalue', rowindx, "qutval", document.getElementById("qutval").value);
	   		 ...
	   		 document.getElementById("prodoc").value="";
	   		 document.getElementById("qutval").value="";
	   		 document.getElementById("saveqty").value="";
		...
		/* 	 document.getElementById("clientname").value="";
		  ...
		  function getBill(){ 
			...
			function getProduct() {
					...
					if (x.readyState == 4 && x.status == 200) {
						...
						var prddatas=JSON.parse(items.trim());          
		  					...
		  					$.each(prddatas, function( index, value ) {
		  						optionsyearnw += '<option value="' + value.part_no + '" data-prdname="'+value.productname+'" data-tax="'+value.taxper+'" data-unit="'+value.unit+'" data-brand="'+value.brandname+'" data-psrno="'+value.psrno+'" data-specid="'+value.specid+'" data-unitdoc="'+value.unitdocno+'" data-uprice="'+0+'" data-taxdoc="'+value.taxdocno+'" data-catname="'+value.category+'" data-scatname="'+value.subcategory+'" data-sprice="'+value.cost_price+'" data-dmbrand="'+value.demobrand+'" data-allowdiscount="'+value.allowdiscount+'"data-qty="'+value.qty+'"data-checktype="'+value.checktype+'"data-productid="'+value.productid+'"data-proid="'+value.proid+'"data-proname="'+value.proname+'"data-sr_no="'+value.sr_no+'"data-prodoc="'+value.prodoc+'"data-qutval="'+value.qutval+'"data-saveqty="'+value.saveqty+'"></option>';
				...
				x.open("GET", '../getProduct.jsp?reqmasterdocno='+reqmasterdocno+'&reftype='+document.getElementById("reftype").value+'&acno='+accdocno+"&dates="+dates+"&cmbbilltype="+cmbbilltype+"&brandids="+brandids+"&id="+id+"&clientcaid="+clientcaid+"&dtype="+dtype+"&presulttype="+presulttype, true);
				x.send();
			}
			...
			function prosrch(){
			     ...
			     var reqmasterdocno = document.getElementById("reqmasterdocno").value;    
			  	 ...
			  	 var dtype='PO';
	 	      	 var cmbbilltype=document.getElementById("cmbbilltype").value;
		   	     productSearchContent('productSearch.jsp?reqmasterdocno='+reqmasterdocno+'&reftype='+document.getElementById("reftype").value+'&acno='+accdocno+"&dates="+dates+"&cmbbilltype="+cmbbilltype+"&dtype="+dtype);
			}
...
<body onLoad="checkReftype();getCurrencyIds();setValues();chkcostcode();getitemtype();getBill();chkLoadFromExcel();">
...
<%--     <td width="1%" align="right">Account</td>
    ...
    <td align="left" width="155">Ref Type
      <select name="reftype" id="reftype" style="width:41%;"  value='<s:property value="reftype"/>' onchange="funrefdisslno()">
        ...
        <option class="refid" value="PR">PR</option>
        <option class="refid" value="SOR">SOR</option>
        <option class="refid" value="RFQ">RFQ</option>
    </select></td>
...
<td  align="right">Net Tax Total</td><td  align="left"><input type="text" id="taxtotal" name="taxtotal" style="width:50%;"   value='<s:property value="taxtotal"/>'  ></td>
 ...
 <input type="hidden" id="tax3per"><!--  tird tax % -->
        ...
        <input type="hidden" id="taxdoc" >  
         ...
         <input type="hidden" id="proname" value="0">  
         <input type="hidden" id="sr_no" value="0">  
         <input type="hidden" id="prodoc" value="0">  
         <input type="hidden" id="qutval" value="0">  
         <input type="hidden" id="saveqty" value="0">  
</form>
</div>
<div id="searchwindow">
   <div ></div>
