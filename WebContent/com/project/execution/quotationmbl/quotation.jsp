...
var mastertrno='<%=mastertrno%>';
	...
	$(document).ready(function() {
	  ...
	  $('#subsertypefowindow').jqxWindow({ width: '30%', height: '65%',  maxHeight: '85%' ,maxWidth: '80%' ,title: 'Sub Service Type' , position: { x: 250, y: 60 }, keyboardCloseKey: 27});
	  ...
	  $('#servtype').load('jqxSearchServType.jsp');
	  $('#unittype').load('jqxSearchUnit.jsp');
	  
	  
	  
	  $('#txtdisper').prop('readonly', false);
	  $('#discountCheck').change(function() {
		    if ($(this).is(':checked')) {
		      $('#txtdisper').prop('readonly', false);
		      $('#txtdisamt').prop('readonly', true);
		      $('#discounttick').val(1);
		    } else {
		      $('#txtdisper').prop('readonly', true);
		      $('#txtdisamt').prop('readonly', false);
		      $('#discounttick').val(0);
		    }
		  });
	  
	  $('input[name="rdo"]').change(function() {
		    if ($(this).is(':checked') && $(this).val() === 'AMC') {
		      $("#amcDetailsField").show();
		    }
		    if ($(this).is(':checked') && $(this).val() === 'SJOB') {
		    	 $("#amcDetailsField").hide();
		    }
		  });
...
function getTemplate(event){
	...
	function getsubSerType(rowBoundIndex,serid){
		 
		  $('#subsertypefowindow').jqxWindow('open');
	//$('#accountWindow').jqxWindow('focus');
	   subSerTypeSearchContent('subservicetypesearch.jsp?rowBoundIndex='+rowBoundIndex+'&servid='+serid); 
	  /* subSerTypeSearchContent('subservicetypesearch.jsp?rowBoundIndex='+rowBoundIndex); */
		 }
	...
	function subSerTypeSearchContent(url) {
	//alert(url);
		 $.get(url).done(function (data) {
			 //alert(data);
	$('#subsertypefowindow').jqxWindow('setContent', data);
	...
	function getclinfo(event){
	 ...
	 function funReadOnly(){
			$('#frmQuotationmbl input').attr('readonly', true );
			$('#ramc').attr('disabled', true );
			$('#rsjob').attr('disabled', true );
			$('#frmQuotationmbl select').attr('disabled', true);
	...
	var docno=$('#docno').val();
	 ...
	 function funRemoveReadOnly(){
			 ...
			 refChange();
			 getPreSalesScopeAllowed();  
				$('#ramc').attr('disabled', false );
				$('#rsjob').attr('disabled', false );
			$('#frmQuotationmbl input').attr('readonly', false );
			$('#frmQuotationmbl select').attr('disabled', false);
			$('#date').jqxDateTimeInput({disabled: false});
			$("#serviceGrid").jqxGrid({ disabled: false});
			$("#siteGrid").jqxGrid({ disabled: false});
			$("#amcGrid").jqxGrid({ disabled: false});
			...
			$('#txtdcdamount').attr('disabled', true);
	 		...
	 		//	$('#txtdisamt').attr('readonly', true );
				...
				if ($("#mode").val() == "A") { 
					...
					$("#siteGrid").jqxGrid('addrow', null, {});
					$("#amcGrid").jqxGrid('clear');

					$("#amcGrid").jqxGrid('addrow', null, {});
	 ...
	 function saveest() {
	  ...
	  function funNotify(){    
			 ...
			 var rows2 = $("#siteGrid").jqxGrid('getrows');
			 var rows3 = $("#jqxTerms").jqxGrid('getrows');
			 var rows4 = $("#amcGrid").jqxGrid('getrows');
			 ...
			 var sietlen=0;   		   
			 for(var i=0; i < rows2.length ; i++){
				    var site = rows2[i].site;
				    if(typeof(site)!="undefined" && typeof(site)!="NaN" && site!="" && site!=null){
				    	newTextBox = $(document.createElement("input"))
					       .attr("type", "dil")
					       .attr("id", "site"+sietlen)
					       .attr("name", "site"+sietlen)     
					       .attr("hidden", "true"); 
			 ...
			 $('#sitelen').val(sietlen);
		     if(sietlen==0){
		    	    document.getElementById("errormsg").innerText="Please enter Site Details!";   
			 		return 0;
		     }

		     var srvlen=0;
			 for(var i=0 ; i < rows1.length ; i++){
				 var stypeid = rows1[i].stypeid;
				 var desc1 = rows1[i].desc1; 
				 //alert("sub type id : "+rows1[i].subservid);
				 if((typeof(stypeid)!="undefined" && typeof(stypeid)!="NaN" && stypeid!="" && stypeid!=null) || (typeof(desc1)!="undefined" && typeof(desc1)!="NaN" && desc1!="" && desc1!=null)){   
				    newTextBox = $(document.createElement("input"))
				       .attr("type", "dil")
				       .attr("id", "service"+srvlen)
				       .attr("name", "service"+srvlen)
				       .attr("hidden", "true"); 
				    
				    newTextBox.val(rows1[i].desc1+" :: "+rows1[i].stypeid+" :: "+rows1[i].unitid+" :: "+rows1[i].qty+" :: "+rows1[i].amount+" :: "+rows1[i].discount+" :: "+rows1[i].disper+" :: "+rows1[i].total+" :: "+rows1[i].revision_no+" :: "+rows1[i].rowno+" :: "+rows1[i].subservid);
				    newTextBox.appendTo('form');
				    srvlen++;   
				 }  
			 }
			 
			 $('#servicelen').val(srvlen);  
		     if(srvlen==0){
		    	    document.getElementById("errormsg").innerText="Please enter Service Details!";      
			 		return 0;
		     }  
		     var amclen=0;

			 for(var i=0 ; i < rows4.length ; i++){
				 var amcdesc = rows4[i].desc1;
				 var amcarea = rows4[i].area;  
				 if((typeof(amcdesc)!="undefined" && typeof(amcdesc)!="NaN" && amcdesc!="" && amcdesc!=null) || (typeof(amcarea)!="undefined" && typeof(amcarea)!="NaN" && amcarea!="" && amcarea!=null)){   
					 newTextBox = $(document.createElement("input"))
				       .attr("type", "dil")
				       .attr("id", "amc"+amclen)
				       .attr("name", "amc"+amclen)
				       .attr("hidden", "true"); 
				    
				    newTextBox.val(rows4[i].desc1+" :: "+rows4[i].area+" :: "+rows4[i].unit+" :: "+rows4[i].unitid+" :: "+rows4[i].rowno);
				    newTextBox.appendTo('form');
				    amclen++;   
				 }  
			 }
	  ...
	  function discountCheckTick() {
		var val=$('#discounttick').val();
		if(val==1){
			document.getElementById("discountCheck").checked = true;
		}
	}
		  ...
		  var reviseno=$("#txtrevise").val();
			...
			if(docno>0){
				...
				refChange();
				 ...
				 $("#amcDetailsDiv").load("amcGrid.jsp?dtype="+dtype+"&qotdoc="+docno);
	...
	<%--   function funPrint(id) {
	 ...
	 /*  function funPrintBtn() {
	  ...
	  function funPrintBtn() {
		  PrintContent('printVoucherWindowMbl.jsp');
		  //	  var docno=$('#docno').val();
	  		// var trno=$('#masterdoc_no').val();
	  	//	var dtype=$('#formdetailcode').val(); 
	  	...
	  	//	var url=document.URL;
	  	//	var reurl=url.split("com/");     
	  	  //  var win= window.open(reurl[0]+"printQuotationmbl?docno="+docno+"&brhid="+brhid+"&trno="+trno+"&dtype="+dtype,"_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
	  ...
	  function getEnquiry(event){
		...
		/* 	var additionamt = $('#txtadditionalamt').val(); */    
	        ...
	        console.log("dis Perc"+discountper);
	        if(grtotal!=''){
	...
	function funconfirm(trno,revision){
	 ...
	 function editstatuscheck(){         
		...
		function prosrch(){
	    	var clientid=document.getElementById("txtclient").value;
	    	
	    	if(clientid==""){
	    		document.getElementById("errormsg").innerText="Select Customer";
	    		$("#txtclient").focus();
	    		return false;
	    	}
	    	document.getElementById("errormsg").innerText=""
		...
		function funSubService() {
			var srid=$('#servtypeid').val();
			$('#subservtype').load('jqxSearchSubServType.jsp?srvid='+srid);
		}
		function clearstock() {
			$('#jqxServType').val('');
			$('#jqxSubServType').val('');
			$('#jqxUnit').val('');
			//$('#jqxServType').removeAttr('value');
			$('#txtqty').val('');
			$('#txtamount').val('');
			$('#txtdescription').val('');
			//$('#subservtype').load('jqxSearchSubServType.jsp?srvid='+srid);
		...
		function setGrid() {
			var desc2 = document.getElementById("txtdescription").value;
			var stype = document.getElementById("jqxServType").value;
			var unit = document.getElementById("jqxUnit").value;
			...
			if(unit==""){
				document.getElementById("errormsg").innerText=" Unit is Mandatory ";
				return 0;
			...
			document.getElementById("errormsg").innerText=" ";
			 ...
			 var desc = rows[index].desc1;
			 var stypeid = rows[index].stypeid;
			 if((typeof(stypeid)=="undefined" || typeof(stypeid)=="NaN" || stypeid=="" || stypeid==null) && (typeof(desc1)=="undefined" || typeof(desc1)=="NaN" || desc1=="" || desc1==null)){
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "desc1", document.getElementById("txtdescription").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "stype", document.getElementById("jqxServType").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "unit", document.getElementById("jqxUnit").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "qty", document.getElementById("txtqty").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "amount", document.getElementById("txtamount").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "stypeid", document.getElementById("servtypeid").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "unitid", document.getElementById("unitid").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "subservname", document.getElementById("jqxSubServType").value);
			     $('#serviceGrid').jqxGrid('setcellvalue',  index, "subservid", document.getElementById("subservtypeid").value);
...
<form id="frmQuotationmbl" action="saveQuotationmbl" method="post" autocomplete="off">    
...
<table width="100%">
  ...
  <td align="right">Sales Man</td>
    <td><input type="text" id="txtsalman" name="txtsalman" style="width:90%;"    value='<s:property value="txtsalman"/>' tabindex="-1"/></td>  
...
<table width="100%" >    
  <tr>
    <td width="5%" align="right"><input type="radio" id="ramc" name="rdo" onchange="funcheck();getsjobtype();" value="AMC">AMC</td>
    <td width="5%" align="center"><input type="radio" id="rsjob" name="rdo" onchange="funcheck();getsjobtype();" value="SJOB">SJOB</td>
    
    <td width="3%" align="right" class="mblfieldshide">Type</td>
    <td width="8%" class="mblfieldshide"><select name="cmbprocess" id="cmbprocess" style="width:100%;" name="cmbprocess" onchange="funcomprehensivetext();funtxtenable(value);"  value='<s:property value="cmbprocess"/>'></select>
	  <input type="hidden" name="hidcmbprocess" id="hidcmbprocess" value='<s:property value="hidcmbprocess"/>' /></td>
      <td width="7%" align="right" class="mblfieldshide"><label id="lblThresholdLimit">Threshold Limit</label></td>
    <td width="7%" class="mblfieldshide"><input type="text" id="txtthresholdlimit" name="txtthresholdlimit" style="width:90%;text-align:right;" onkeypress="javascript:return isNumber (event,id)" value='<s:property value="txtthresholdlimit"/>'/></td>
    <td width="7%" align="right" class="mblfieldshide"><label id="lblPartLimitPerc">Part Limit %</label></td>
    <td width="7%" class="mblfieldshide"><input type="text" id="txtpartlimitperc" name="txtpartlimitperc" style="width:90%;text-align:right;" onkeypress="javascript:return isNumber (event,id)" onblur="funmaxlength(this.value,this.id);" value='<s:property value="txtpartlimitperc"/>'/></td>
    <td width="6%" align="right" class="mblfieldshide"><input type="checkbox" name="chklegaldoc" id="chklegaldoc"  onchange="legChange();"> &nbsp;Legal Fee</td>
    <td width="8%" class="mblfieldshide"><input type="text" id="txtdcdamount" name="txtdcdamount" style="width:90%;text-align: right;" value='<s:property value="txtdcdamount"/>' onblur="funRoundAmt(this.value,this.id);" /></td>
    ...
    <td width="5%"><select id="cmbreftype" name="cmbreftype" style="width:97%;" onchange="refChange();" value='<s:property value="cmbreftype"/>'>
     ...
     <td width="10%"><input type="text" id="txtenquiry" name="txtenquiry" style="width:90%;" placeholder="Press F3 to Search" value='<s:property value="txtenquiry"/>' onKeyDown="getEnquiry(event);"/></td>
      <td align="right" width="7.6%">Subject</td>
    <td  width="20%"><input type="text" id="txtsubject" name="txtsubject" style="width:72.2%;" value='<s:property value="txtsubject"/>'/></td>
  </tr>
  <tr>
    <td align="right">Description</td>
    <td colspan="4"><input type="text" id="txtdesc1" name="txtdesc1" style="width:96.3%;" value='<s:property value="txtdesc1"/>'/></td>
    <td align="right">Client Ref.</td>
    <td  width="20%"><input type="text" id="txtclientref" name="txtclientref" style="width:72.2%;" value='<s:property value="txtclientref"/>'/></td>
  </tr>
  <tr>
  <td align="right">Notes</td>
    <td colspan="6"><input type="text" id="txtnotes" name="txtnotes" style="width:89.7%;" value='<s:property value="txtnotes"/>'/></td>
  </tr>
</table>
</fieldset>
...
<fieldset id="amcDetailsField" ><legend><font style="font-family: comic sans ms;font-weight: bold;">AMC Details</font></legend>
<div id="amcDetailsDiv"><jsp:include page="amcGrid.jsp"></jsp:include></div>
</fieldset>


 
<fieldset ><legend><font style="font-family: comic sans ms;font-weight: bold;">Service Details</font></legend>
   <table width="100% " id="psearch" >   
   <tr> 
   <td align="center">Description</td>
   <td align="center">Service Type</td>
   <td align="center">Sub Service Type</td>
   <td align="center" style="width:15%;" >Unit</td>
   <td align="center">Qty</td>
   <td align="center">Amount</td> 
   </tr>
    <tr>
   <td width="20% " ><input type="text" class="form-control" id="txtdescription"  style="text-align: left;width:96%"></td>
<td width="15% " ><div id="servtype"><jsp:include page="jqxSearchServType.jsp"></jsp:include></div>
<input type="hidden" name="servtypeid" id="servtypeid" > </td>
<td width="15% " ><div id="subservtype"><jsp:include page="jqxSearchSubServType.jsp"></jsp:include></div>
<input type="hidden" name="subservtypeid" id="subservtypeid" > </td>
 <td align="center" > <div id="unittype"><jsp:include page="jqxSearchUnit.jsp"></jsp:include></div>
<input type="hidden" name="unitid" id="unitid" />   </td>
<td align="center"><input type="text" id="txtqty"   > </td>
<td align="center"><input type="text" id="txtamount" style="text-align: right;"   ></td>
   <td align="center">
     <input type="hidden" id="cleardata">
 &nbsp; <input type="button" id="setbtn"  class="btn" onclick="setGrid()" value="ADD" ></td>
 <td align="center"><input type="hidden" id="cleardata">&nbsp; <input type="button" id="setbtn1"  class="btn" onclick="clearstock()" value="Clear" ></td>
 </tr>
</table> 
...
<table width="100%">   
  <tr>
    ...
    <td width="1%"><input type="checkbox" id="discountCheck" /> </td>
...
<input type="text" id="hidradio" name="hidradio"  value='<s:property value="hidradio"/>'/>
...
<input type="hidden" id="amclen" name="amclen"  value='<s:property value="amclen"/>' />
<input type="hidden" id="discounttick" name="discounttick"  value='<s:property value="discounttick"/>' />
</div>
