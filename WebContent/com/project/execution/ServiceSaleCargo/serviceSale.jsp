							...
							else
							{
							 ...
							 for(var i=0 ; i < rows.length ; i++){
								 ...
								 newTextBox.val(rows[i].srno+"::"+rows[i].qty+" :: "+rows[i].description+" :: "
										 ...
										 +rows[i].costtype+" :: "+rows[i].costcode+" :: "+rows[i].remarks+" :: "+rows[i].headdoc+" :: "+aa+" :: "+rows[i].curid+" :: "+rows[i].rate+" :: ");                                      
							 ...
							 x.onreadystatechange=function()
							 {
								 if(x.readyState==4 && x.status==200)	
								 {
									 ...
									 {
										 document.getElementById("errormsg").innerText="";
										 document.getElementById("frmNipurchasecargo").submit();
									 }
									 else
									 {
										 document.getElementById("errormsg").innerText="";
 ...
 */			
			...
			function funSearchLoad(){
				...
				function funReset(){
					//$('#frmNipurchasecargo')[0].reset(); 
				}
				function funReadOnly(){
					$('#frmNipurchasecargo input').attr('readonly', true );
					$('#frmNipurchasecargo select').attr('disabled', true );
				...
				function funRemoveReadOnly(){         
					...
					fungetterms();
					getNonTaxableEntity();
					$('#frmNipurchasecargo input').attr('readonly', false );
					$('#frmNipurchasecargo select').attr('disabled', false );
	...
	function funPrintBtn(){
	   var dtype=$('#formdetailcode').val();
		...
		if( url.indexOf('savesrvsalecargo') >= 0){
			reurl=url.split("savesrvsalecargo");
		}else {
		...
		if (($("#mode").val() == "view") && $("#masterdoc_no").val()!="" && $("#hidbnk").val()==1) {
			...
			var win= window.open(reurl[0]+"PRINTServiceSalecargo?docno="+document.getElementById("masterdoc_no").value+"&brhid="+brhid+"&dtype="+dtype+"&header=1&bankdocno=0","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=yes,toolbar=yes");
			win.focus();
   ...
   function PrintContent(url) {
	...
	<form id="frmNipurchasecargo" action="savesrvsalecargo" method="post" autocomplete="off">
