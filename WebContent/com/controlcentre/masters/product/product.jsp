...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#specmasterssearchwindow').jqxWindow({
			width : '25%',
			height : '58%',
			maxHeight : '70%',
			maxWidth : '45%',
		...
		document.getElementById('warrantydays').addEventListener('input', function (event) {
		      // Get the input value
		...
		$('#txthsn').dblclick(function(){
			hsnFormSearchContent('hsnFormSearchGrid.jsp');
		}); 
	...
	function specMastersSearchContent(url) {
		$('#specmasterssearchwindow').jqxWindow('open');
		$.get(url).done(function(data) {
			$('#specmasterssearchwindow').jqxWindow('setContent', data);
			$('#specmasterssearchwindow').jqxWindow('bringToFront');
		});
	}
	...
	function hsnFormSearchContent(url) {
			$('#hsnsearchwindow').jqxWindow('open');
			$.get(url).done(function(data) {
				$('#hsnsearchwindow').jqxWindow('setContent', data);
				$('#hsnsearchwindow').jqxWindow('bringToFront');
			});
		}
...
/* function funprddel(){
	...
	function funRemoveReadOnly() {
		...
		$('#warrantydays').attr('readonly', false);
		$('#frmProduct input').attr('disabled', false);
		...
		if ($("#mode").val() == "A") {
			...
			$("#jqxspecMastersGrid").jqxGrid('clear');
			$("#jqxspecMastersGrid").jqxGrid('addrow', null, {});
			...
			getpmconfig();
			$("#specMastersDiv").load("specMastersGrid.jsp?docno="+0+"&detachid="+$('#hiddetachconfig').val()+"&id="+1);
			$("#jqxspecMastersGrid").jqxGrid({disabled:false});
	 	...
	 	if ($("#mode").val() == "E") {
			...
			var docno=document.getElementById("docno").value;
			var deptid=document.getElementById("cmbdept").value;
			if(docno>0){
				...
				$("#specMastersDiv").load("specMastersGrid.jsp?docno="+docno+"&deptid="+deptid+"&detachid="+$('#hiddetachconfig').val()+"&id="+1);
			}
			$("#jqxSuitGrid").jqxGrid({
	  			disabled : false
	  		});
	      ...
	      /* if(rowlength==1){
		   		$('#jqxspecMastersGrid').jqxGrid('setcellvalue'," ", 'description'," ");
	   	  }  */ 
	      }
	...
	function getClassificationGridConfig(){
		var x=new XMLHttpRequest();
		x.onreadystatechange=function(){
		if (x.readyState==4 && x.status==200)
			{
	     ...
	     x.open("GET",'classificationGridConfig.jsp',true);
	     x.send();   
	}
		...
		if (x.readyState==4 && x.status==200)
			{
			 	var res= x.responseText;
			 	if(res>0){
			 		document.getElementById("hiddetachconfig").value = "DET"; 
				} else {
					  $('.detach').hide();
				}
			} else {
	     ...
	     x.open("GET",'getconfigdeptdetach.jsp',false);
	     x.send();   
	}
		...
		if (x.readyState==4 && x.status==200)
			{
			 	var res= x.responseText;
			 	if(res>0){
			 		$("#subcatsearchmain").hide();
			 		$("#subcat").hide();
			 		$("#subcatconfig").hide();
			 		$("#subcatconfigtext").hide();
				}
			}
		       else
			  {}
	...
	/* 	
		...
		if($('#hidewarranty').val()!=""){
			$('#warranty').val($('#hidewarranty').val());
		}
		
		if($('#hidewarrantydays').val()!=""){
			$('#warrantydays').val($('#hidewarrantydays').val());
		}
	...
	function fungridload(){
		...
		var docno=document.getElementById("docno").value;
		var deptid=document.getElementById("cmbdept").value;
		var detachid = $('#hiddetachconfig').val();
		...
		$("#specMastersDiv").load("specMastersGrid.jsp?docno="+docno+"&deptid="+deptid+"&detachid="+detachid+"&id="+1);
	...
	function funNotify() {
		...
		if(parseFloat($("#hidpartnoduplicate").val())==1){
		...
		var specMastersRows = $("#jqxspecMastersGrid").jqxGrid('getrows');
		...
		if (!((uomrows[0].unitid == "undefined") || (uomrows[0].unitid == null) || (uomrows[0].unitid ==" "))) {
			...
			for (var i = 0; i < uomrows.length; i++) {
				...
				newTextBox.val(uomrows[i].unitid + "::" + uomrows[i].fr + "::"
						...
						+ "::" + uomrows[i].width + "::" + uomrows[i].docno + "::");
/* (uomrows[i].unitid + "::" + uomrows[i].fr + "::"
		+ uomrows[i].weight + "::" + uomrows[i].volumn + "::"
		+ uomrows[i].thickness + "::" + uomrows[i].len
		+ "::" + uomrows[i].width + "::"); */
				newTextBox.appendTo('form');
		...
		for (var i = 0; i < alterrowsk.length; i++) {
			...
			//alert("psrno===="+alterrowsk[i].psrno);
		...
		if(specMastersRows.length>0){
		if (!((specMastersRows[0].name == "undefined") || (specMastersRows[0].name == null) || (specMastersRows[0].name ==" "))) {
			$('#specMastersGridlen').val(specMastersRows.length);
			
			for (var i = 0; i < specMastersRows.length; i++) {
				//	var myvar = rows[i].tarif; 
				newTextBox = $(document.createElement("input")).attr("type",
						"dil").attr("id", "specMasters" + i).attr("name", "specMasters" + i)
						.attr("hidden", "true");
				newTextBox.val(specMastersRows[i].name + "::" + specMastersRows[i].description + "::"+ specMastersRows[i].sdoc_no + "::");
		...
		if (!((specrows[0].spec1 == "undefined") ||  (specrows[0].spec1 ==0) || (specrows[0].spec1 == "0") ||  (specrows[0].spec1 == typeof("undefined")) ||  (specrows[0].spec1 == typeof(null))  ||  (specrows[0].spec1 == typeof("")) || (specrows[0].spec1 == null) || (specrows[0].spec1 == "")))  {
	...
	function checkpartnodup(){
    	...
    	var docno=$.isNumeric(document.getElementById("docno").value)?document.getElementById("docno").value:0;
    	var x=new XMLHttpRequest();
		x.onreadystatechange=function(){
		if (x.readyState==4 && x.status==200){
			var items= x.responseText.trim();
			...
			if(parseInt(items)==1){
				$("#hidpartnoduplicate").val(1);
				document.getElementById("errormsg").innerText ="Part_no Already Exists...!!!";
				return 0;
   	        }else {
   	        	$("#hidpartnoduplicate").val(0);
				document.getElementById("errormsg").innerText ="";
			}
   	    }}
		x.open("GET","checkduplcte.jsp?partno="+partno.trim()+'&docno='+docno,true);
   		x.send();	
    }
...
<body
	onload="setValues();getpmconfig();getAlterConfig(); getClassificationGridConfig(); getsubcatConfig(); getcatConfig();gethsnConfig();">
					...
					<tr>
						<td width="40%">
							<fieldset>
								<table width="100%">
									<tr>
										<td width="30%" align="right">Product Code</td>
										<td width="70%"><input type="text" id="txtproductcode"
									...
									<tr>
										...
										<td>
										<input type="text" id="txtbarcode" name="txtbarcode"style="width: 20%;" value='<s:property value="txtbarcode"/>' />
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										    Star 
											<select  name="cmbstar" id="cmbstar"  value='<s:property value="cmbstar"/>' style="width:31%;">
    									...
    									<label id="mtypess">Type</label> 
       									<select  name="cmbmastertype" id="cmbmastertype"  value='<s:property value="cmbmastertype"/>' style="width:30%;"><option ></option></select> </td> 
									</tr>
  ...
  <tr>  
       <td width="30%">
							...
							<fieldset style="margin-top:16px;">
								<table width="100%">
									...
									<tr>
										 ...
										 <td align="right" id="catconfig">Category</td>
										<td id="catconfigtext"><input type="text" id="txtcategory" name="txtcategory"
										...
										<td align="right" id="subcatconfig">Sub Category</td>
										<td id="subcatconfigtext">
									...
									<tr>
										...
										<tr>
										<td align="right" class="hsnconfighid">HSN Code</td>
										<td>
											<input type="text" id="txthsn" class="hsnconfighid" name="txthsn"
											style="width: 50%;" placeholder="Press F3 for Search" readonly="true" onKeyDown="getProdHsn(event);" value='<s:property value="txthsn"/>' />	
											<input
											...
											<input type="hidden" id="hiddetachconfig" name="hiddetachconfig" value='<s:property value="hiddetachconfig"/>' />
											<input type="hidden" id="cmbhsn" name="cmbhsn" value='<s:property value="cmbhsn"/>' />
											...
											<input type="hidden" id="specMastersGridlen" name="specMastersGridlen" value='<s:property value="specMastersGridlen"/>' />
									...
									<tr class="detach">
										...
										<td>
											<select  name="warranty" id="warranty"  value='<s:property value="warranty"/>' style="width:31%;">
												<option value="">--select--</option>
												<option value="YES">YES</option>
												<option value="NO">NO</option>
											</select>
										</td>
										<td align="right">Warranty Days</td>
										<td>
											<input type="text" id="warrantydays" name="warrantydays"  style="width: 20%;"  value='<s:property value="warrantydays"/>'/>
										</td>
									</tr>
				...
				<table width="100%" style="margin-top:-10px;">
					<tr>
						<td>
							<div id="classification" style="margin-top:-5px;">
								<fieldset>
									<legend>Classification Setup</legend>
									<div id="specMastersDiv">
									<jsp:include page="specMastersGrid.jsp"></jsp:include></div>
								</fieldset>
							</div>
						</td>	
					</tr>
				...
				<input type="hidden" id="hidewarranty" name="hidewarranty" value='<s:property value="hidewarranty"/>' />
				<input type="hidden" id="hidewarrantydays" name="hidewarrantydays" value='<s:property value="hidewarrantydays"/>' />
			...
			<input type="hidden" id="hidpartnoduplicate" name="hidpartnoduplicate" value='<s:property value="hidpartnoduplicate"/>' /> <!-- //div -->
		...
		<div id="specmasterssearchwindow">
			<div></div>
			<div></div>
		</div>
