     ...
     $(document).ready(function () { 	
            ...
            $("#cpDetailsGrid").jqxGrid(
            {
                ...
                var rowlength= rows.length;
                   ...
                   if (cell != undefined && cell.datafield == 'cpersion' && cell.rowindex == rowlength - 1) {
                       var key = event.charCode ? event.charCode : event.keyCode ? event.keyCode : 0; 
                       if (key ==13 || key == 114 || key == 9) {      
                           var commit = $("#cpDetailsGrid").jqxGrid('addrow', null, {});
                           rowlength++; 
                       }
                   }   
                ...
                columns: [
						...
						{ text: 'Row No',  datafield: 'row_no',  width: '30%'  ,editable:true ,renderer: columnsrenderer, cellsalign: 'center', align: 'center',cellclassname:'column',hidden:true},
            ...
            // });
	            ...
	            $("#cpDetailsGrid").on('cellendedit', function (event) {
	                var columnDataField = event.args.datafield;
	                var rowIndex = event.args.rowindex;
	                var cellValue = event.args.value;
	                var oldValue = event.args.oldvalue;
	                if(columnDataField=="cpersion"){   
	                	 $('#cpDetailsGrid').jqxGrid('setcellvalue', rowIndex, 'cpersion', oldValue);
	                }
	            });
        });
