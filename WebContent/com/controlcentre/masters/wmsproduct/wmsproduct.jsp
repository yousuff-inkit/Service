...
<script type="text/javascript">
	...
	/* Validations */
	$(function(){
	    $('#frmWmsproduct').validate({
	             rules: {
	            	txtproductcode: {required:true},
	            	txtproductname : {required:true},
	    			/* 	txtbarcode : "required", */
	    				txtproducttype :  {required:true},
	    				txtbrand :  {required:true},
	    				txtcategory :  {required:true},
	    				txtsubcategory :  {required:true},
	    				txtunit :  {required:true},
	    				textdept : {required:true},
	             },
	             messages: {
	     			...
	     			/* 	txtbarcode : " *", */
	     				...
	     				txtsubcategory :{required:" *"},
	     				txtunit : {required:" *"},
	     				txtdept : {required:"*"},
	               }
	    });});
	function funReadOnly() {
		$('#frmWmsproduct input').attr('readonly', true);
		$('#frmWmsproduct input').attr('disabled', true);
...
/* function funprddel(){
	...
	var docno=document.getElementById("docno").value;
		...
		x.onreadystatechange = function() {
			...
			if(parseInt(items)>0)
				{
				$('#frmWmsproduct input').attr('disabled', true);
				$('#frmWmsproduct select').attr('disabled', true);
			...
			//	alert("34567890-");
				...
				$('#frmWmsproduct select').attr('disabled', false);
				$('#mode').val('D');
				$('#frmWmsproduct').submit();
	...
	function funRemoveReadOnly() {
		...
		$('#txtjde').attr('readonly', false);
		$('#txtbh').attr('readonly', false);
		$('#frmWmsproduct input').attr('disabled', false);
		...
		if ($("#mode").val() == "A") {
			...
			$("#jqxWmsproductGrid").jqxGrid('clear');
			$("#jqxWmsproductGrid").jqxGrid('addrow', null, {});
			...
			$("#wmsproductDiv").load("wmsproductGrid.jsp?chktype="+1);
	 	...
	 	if ($("#mode").val() == "E") {
			...
			if(docno>0){
				...
				{
					$("#wmsproductDiv").load("wmsproductGrid.jsp?docno="+docno+"&chktype="+1);
				}
			else if(document.getElementById("comorbranch").value=="2")
				{
				$("#wmsproductDiv").load("wmsproductGrid.jsp?docno="+docno+"&chktype="+2);
				}	
			}
			$("#jqxSuitGrid").jqxGrid({
	  			disabled : false
	...
	function funNotify() {
		...
		var prorows = $("#jqxWmsproductGrid").jqxGrid('getrows');
...
/* alert(uomrows[i].unitid + "::" + uomrows[i].fr + "::"
	...
	function funcomorbranch()
		...
		if(document.getElementById("company").checked == true)
			...
			{
			$("#wmsproductDiv").load("wmsproductGrid.jsp?chktype="+2);
...
<body
	...
	<div id="mainBG" class="homeContent" data-type="background">
		<form id="frmWmsproduct" action="saveWmsproduct" method="post"
			autocomplete="off">
							...
							<fieldset>
								<table width="100%">
									...
									<tr>
										...
										<td colspan="2" ><input type="text" id="txtbarcode" name="txtbarcode" style="width: 30%;" value='<s:property value="txtbarcode"/>' />
										 JDE No<input type="text"  id="txtjde" name="txtjde" style="width: 40%;" value='<s:property value="txtjde"/>' />
									</td></tr>
 <tr>
    ...
    BH No<input type="text"  id="txtbh" name="txtbh" style="width: 40%;" value='<s:property value="txtbh"/>' /></td>
					...
					<tr>
						<td colspan="4">
							<fieldset>
									...
									<div id="wmsproductDiv">
									<jsp:include page="wmsproductGrid.jsp"></jsp:include><br /></div>
								</fieldset>
