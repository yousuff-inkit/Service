...
$(document).ready(function () {  
	         var cntdata = '<%=DAO.serviceGrid(id,trno) %>';       
             var num = 1; 
            // prepare the data
           ...
           var rendererstring1=function (aggregates){
            ...
            var cellclassname = function(row, column, value, data){
         	   var amountlc = $('#jqxServiceYearlyGrid').jqxGrid('getcellvalue', row, "amountlc");
         	   if(parseFloat(amountlc) > 0) {   
         		    return "colorClass1";  
         	   } 
            }   
            
            $("#jqxServiceYearlyGrid").jqxGrid(
            {
                width: '100%',
                height: 170,
                source: dataAdapter,
                ...
                columns: [
    	                	...
    	                	{ text: 'Bill Method', datafield: 'billmethodname', width: '10%',editable:false,cellclassname:cellclassname },     
            ...
            $('#jqxServiceYearlyGrid').on('celldoubleclick', function(event) { 
            	var rowBoundIndex = event.args.rowindex;
            	var datafield = event.args.datafield;
 		        if(datafield=="curr") {    
 		    	 	getcurr(rowBoundIndex,1);        
	    	    }
 		   });
            $('#jqxServiceYearlyGrid').on('cellvaluechanged', function(event) { 
            	var rowBoundIndex = event.args.rowindex;
            	var datafield = event.args.datafield;
            	var rate =  $("#txtrate1").val();
            	var amounttr =  $("#jqxServiceYearlyGrid").jqxGrid('getcellvalue',rowBoundIndex,'amounttr');
 		        if(datafield=="amounttr") {    
 		    	 ...
 		    	 $("#jqxServiceYearlyGrid").jqxGrid('setcellvalue',rowBoundIndex,'amountlc',parseFloat(rate)*parseFloat(amounttr));  
	    	    }    
 		   });  
        });  
    </script>
    <div id="jqxServiceYearlyGrid"></div> 