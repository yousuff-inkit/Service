        ...
        public JSONArray getZonSalRept(String from, HttpServletRequest request,String branch,String amount1,String amount2)throws SQLException{
			...
			System.out.println("=====amount===="+amount1+"=======amount2====="+amount2);
				...
				if(!(amount1.equalsIgnoreCase(""))&&!(amount2.equalsIgnoreCase(""))){
					sqltst2=" where g.lastyearcurmonthamount between "+amount1+" and "+amount2+" and g.curyearcurmonthamount between "+amount1+" and "+amount2+"";
					}
				/* if(!(to.equalsIgnoreCase("undefined"))&&!(to.equalsIgnoreCase(""))&&!(to.equalsIgnoreCase("0"))){
					sqltodate=cm.changeStringtoSqlDate(to);
					} */
			   ...
			   String temp="";
				...
				while(resultSet.next()){
						...
						temp=rowcount+"::"+resultSet.getString("branchname")+"::"+
...
public JSONArray getUtilization(String from,String to,String jobno,String empid,String chk, HttpServletRequest request)throws SQLException{
	...
	try {
		...
		/* String strSql="select concname,ts.costtype,ts.costcode,sum(ts.labourhrs) labourhrs,CONVERT(concat(Floor(sum(ts.labourhrs)/60),':',(sum(ts.labourhrs)-(Floor(sum(ts.labourhrs)/60)*60))),CHAR(100)) labourhrstime,sum(ts.labourvalue) labourvalue, "
				+ "name from (select e.name,convert(concat(e.name,'-',ts.costcode),char(50))concname,ts.costtype,ts.costcode,coalesce(normal,'0') labourhrs, round((SUBSTRING_INDEX(normalhrs,':',1) * ts.costperhour) + ((SUBSTRING_INDEX(normalhrs,':',-1)/60)*  ts.costperhour),2) labourvalue from  hr_timesheethrs ts "
			   + "left join hr_empm e on e.doc_no=ts.empid left join (select catid,max(doc_no) doc from hr_paycode where status=3 group by catid) c on e.pay_catid=c.catid  left join hr_paycode pc on e.pay_catid=pc.catid and pc.status=3 and c.doc=pc.doc_no where ts.costtype in (3,4) "+sqltst+") ts group by ts.costtype,ts.costcode"; */
