...
ClsCommon ClsCommon=new ClsCommon();
	...
	try{
		...
		String enqdocno=request.getParameter("enqdocno").trim();
...
if(!(areaidnw.equalsIgnoreCase(""))){
 ...
 if(!(salidnw.equalsIgnoreCase(""))){
	salid=Integer.parseInt(salidnw);
		...
		int val=0,reqval=0;
		 /*Submit */
		reqval=DAO.insert(sqlDate, client, 1, 0, salid, category, "0", tinno, 0.0, 0.0, 0.0, Txtaddress, "0", Txtmobile, "0", "0", "0", Txtemail, cpersonid, areaid, "0", "0", "0", "0", "0", "0", "0", 0, "0", cparrayList, session, "A", "0", "0", "0",request, 1, 0, documentsarray);
		
		if(reqval>0){
			sql="update gl_enqm set cldocno="+reqval+" where doc_no="+enqdocno ;
			 System.out.println("update gl_enqm=="+sql);
	    	val= stmt.executeUpdate(sql);
	    	sqlsub="update cm_srvqotm set cldocno="+reqval+" where refdocno="+enqdocno+" and ref_type='enq'" ;
			 System.out.println("update cm_srvqotm=="+sqlsub);
