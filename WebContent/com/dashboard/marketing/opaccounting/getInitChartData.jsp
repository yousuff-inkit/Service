...
<%
	...
	try{
			...
			String strenq="select (select count(*) from cm_srvcontrm m left join cm_srvcontrpd d on d.tr_no=m.tr_no where m.dtype='SJOB' and m.status=3 and m.jbaction in (0,4) and m.pstatus=1 and coalesce(m.pinvno,0)=0 and d.dueafser=98 "+sqltest+") proforma,(select count(*) from cm_srvcontrm m left join cm_srvcontrpd d on d.tr_no=m.tr_no where m.dtype='SJOB' and m.status=3 and m.jbaction in (0,4) and m.pstatus=1 and coalesce(m.pinvno,0)!=0 and d.dueafser=98 "+sqltest+") notrcvd,(select count(*) from cm_srvcontrm m left join cm_srvcontrpd d on d.tr_no=m.tr_no where m.dtype='SJOB' and m.status=3 and d.invtrno>0 and d.confirm=0 and m.jbaction in (0,4) "+sqltest+") tobecnfmed";
