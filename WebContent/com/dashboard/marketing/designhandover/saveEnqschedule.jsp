...
<% 
 ...
 try{
		...
		String sqlss="insert into gl_enqfollowup( rdocno, brhid,userid,description,date,fdate,type) values ("+docno+","+branchid+",'"+userid+"','"+description+"',now(),'"+sdate+"',"+(process.equalsIgnoreCase("FUP")?1:(process.equalsIgnoreCase("DHO")?2:0))+") ";
		...
		if(process.trim().equalsIgnoreCase("DHO")){
			strupSql = "update gl_enqm set designstat=1 where doc_no="+docno+" and status=3 ";      
			rs = stmt.executeUpdate(strupSql);
		}	
