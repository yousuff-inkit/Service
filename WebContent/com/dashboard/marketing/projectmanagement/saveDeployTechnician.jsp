...
<%
	...
	try{
        ...
        while (resultSet.next()) {
	     ...
	     sql1="insert into cm_deploytechm(doc_no, date, costtype, costcode, budget, consumed, balance, brhid, userid, status, reftrno) values('"+docNo+"', now(), '"+costtype+"', '"+costcode+"', '"+budget+"', '"+consumed+"', '"+balance+"', '"+brchid+"', '"+session.getAttribute("USERID").toString()+"', 3, '"+trno+"')";  
		 ...
		 if(dataExist==0){
		     String logdesc="Deploy Technician ###"+trno;                       
			 String sqllog="insert into gl_jobactivitylog(doc_no, brhId, dtype, edate, userId,description) values("+vocno+",'"+brchid+"','PJM',now(),'"+session.getAttribute("USERID")+"','"+logdesc+"')";                       
