...
$(document).ready(function () {     	
            ...
            $("#jqxrouteGrid").jqxGrid(  
            {
                ...
                columns: [
                            ...
                            { text: 'routeid', datafield: 'routeid',editable:false, width: '4%',hidden:true}, 
			     ]
            });
            $("#overlay, #PleaseWait").hide(); 
            $("#jqxrouteGrid").on('rowselect', function () {
                var rowindexes = $('#jqxrouteGrid').jqxGrid('getselectedrowindexes');
                //alert(rowindexes[0]+"Selected Row Indexes: " + rowindexes);
                
                document.getElementById("txtroute").value = $('#jqxrouteGrid').jqxGrid('getcellvalue', rowindexes[0], "route");
                document.getElementById("hidrouteid").value = $('#jqxrouteGrid').jqxGrid('getcellvalue', rowindexes[0], "routeid");
            });
            $('#jqxrouteGrid').on('rowdoubleclick', function (event) {           
           	  var rowindex1=event.args.rowindex;    
           	  var delid =$('#jqxrouteGrid').jqxGrid('getcellvalue', rowindex1, "delid");
          	  if(parseInt(delid)>0){
          		$('#hiddelid').val(delid);  
          	  }else{
          		$('#hiddelid').val("0");  
