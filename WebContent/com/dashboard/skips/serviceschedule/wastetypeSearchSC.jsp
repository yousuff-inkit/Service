...
#jqxwastetypeSC{
	...
	width: 93.5%;
       ...
       <script type="text/javascript">
		...
		$(document).ready(function () { 	   
            ...
            $("#jqxwastetypeSC").jqxInput({ source: dataAdapter, displayMember: "name", valueMember: "name",placeHolder: "Please enter waste type"});
            $("#jqxwastetypeSC").on('select', function (event) {            
            	  if (event.args) {
                      var item = event.args.item;           
                      if (item) {
                          for (var i = 0; i < dataAdapter.records.length; i++) {        
                              if (item.value == dataAdapter.records[i].name) {   
                                  document.getElementById("scwasteid").value=dataAdapter.records[i].doc_no;           
                                  break;   
                              }   
                          }
                      }
    ...
    <input id="jqxwastetypeSC" class="p-l-5 input-sm form-control"/>     