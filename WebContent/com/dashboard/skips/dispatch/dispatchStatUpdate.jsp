...
try{          
	...
	if(!(status.equalsIgnoreCase("Complete"))){                  
		if(!asgnuser.equalsIgnoreCase("")){              
			strsql="update in_dispatchcreation set ass_user='"+asgnuser+"',act_status='"+status+"' where doc_no='"+docno+"' ";	
			val=stmt.executeUpdate(strsql);
		}else{  
			strsql="update in_dispatchcreation set ass_user='"+oldassuser+"',act_status='"+status+"' where doc_no='"+docno+"'";
			val=stmt.executeUpdate(strsql);
		}      
	}else if(status.equalsIgnoreCase("Complete")){          
			strsql="update in_dispatchcreation set ass_user='"+crtuser+"',act_status='"+status+"',close_status=1 where doc_no='"+docno+"' ";	
			val=stmt.executeUpdate(strsql);

			/* if(oldstatus.equalsIgnoreCase("Complete")){       
	    	 val=-888;          
	     }else{} */
	}
	if(val>0){  
		String flwsql="insert into in_dispatchcreationd(rdocno,ass_date,userid,assnfrom_user,action_status,remarks) values('"+docno+"',now(),'"+asgnuser+"','"+session.getAttribute("USERID")+"','"+status+"','"+remark+"')";
