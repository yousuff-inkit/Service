	     ...
	     $("#ackdate").jqxDateTimeInput({ width: '125px', height: '15px',formatString:"dd.MM.yyyy"});
	       ...
	       // $('#ackdate').jqxDateTimeInput('setDate', new Date());  
			...
			$('#todate').on('change', function (event) {
				var fromdates1 = new Date($('#fromdate').jqxDateTimeInput('getDate'));
				var todates1 = new Date($('#todate').jqxDateTimeInput('getDate'));
					   if(todates1<fromdates1){
						   Swal.fire({  
			        			icon: 'warning',
								title: 'Warning',
...
//     	var docno=document.getElementById("docno").value;
    	...
    	if(dispatch=="" &&  collect=="")
    	{
    		Swal.fire({
				icon: 'warning',
				title: 'Warning',
				text: 'Enter Dispatch or collect details'
			});
...
//docno,sdate,stime,asuser,ashiduser,client,hidclient,location,contact,dispatch,collect,msg,userid,hiduser
	 ...
	 function funAttachs(event){                         
	   ...
	   function intro1check(){
		 ...
		 function getOldDetails(docno){    
			 var x = new XMLHttpRequest();
				x.onreadystatechange = function() {
					if (x.readyState == 4 && x.status == 200) {
						var items = x.responseText.trim().split('####');
						$('#cmbmode').val('').trigger('change'); 
						$('#receivedby').val('');
						$('#disuser').val(''); 
						$('#jqxInputUsersss').val('');
						$('#remark').val('');
						$('#ddate').val(new Date());
						$('#ackdate').val(new Date());
						if(parseInt(items[4])>0){
							    $('#cmbmode').val(items[4]).trigger('change');  
							    $('#disuser').val(items[1]);
							    $('#jqxInputUsersss').val(items[0]);    
							    $('#receivedby').val(items[5]);
							    $('#remark').val(items[2]); 
							    $('#ddate').val(items[6]);
							    $('#ackdate').val(items[7]);
						}
				...
				x.open("GET", "getOldDetails.jsp?docno="+docno, true);        
				x.send();
		 }    
  </script>
</body>
