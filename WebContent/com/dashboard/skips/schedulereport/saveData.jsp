...
<%	
    ...
    try{       
		...
		if(processid.equalsIgnoreCase("Status Update")){
				...
				if(!statusid.equalsIgnoreCase("") && !statusid.equalsIgnoreCase("0")){
					for(int i=0;i<docsar.size();i++){ 
						 ...
						 if(statusid.equalsIgnoreCase("1")){
							 ...
							 String upsql2="update sk_srvcontrsch set status=4 where pskipno='"+pskipno+"' and date>=curdate()";      
					    	 //System.out.println("upsql2--->>>"+upsql2);        
							 stmt.executeUpdate(upsql2);
							 
							 String upsql3="update sk_srvcontrsch sc left join sk_srvcontrdel dl on dl.doc_no=sc.delid set sc.status=4 where dl.skipid='"+pskipno+"' and sc.date>=curdate()";
			...
			for(int i=0;i<docsar.size();i++){           
				 String upsql="update sk_srvcontrsch sc left join sk_srvcontrdel dl on dl.doc_no=sc.delid set sc.routeid='"+routeid+"',sc.driverid='"+driverid+"',sc.helperid='"+helperid+"',sc.helperid2='"+helperid2+"',sc.fleetno='"+fleetno+"',dl.routeid='"+routeid+"',dl.driverid='"+driverid+"',dl.helperid='"+helperid+"',dl.helperid2='"+helperid2+"',dl.fleetno='"+fleetno+"' where sc.doc_no='"+docsar.get(i)+"'";   
		    	 //System.out.println("upsql--->>>"+upsql);         
				 val= stmt.executeUpdate(upsql);        
			}
			if(val>0){
		    	 ...
		    	 String days="";
		    	 String strsql1="select brhid,dayname('"+sqldate+"') dayname from sk_srvcontrm where tr_no='"+sccnttrno+"'";   
				 //System.out.println("strsql1--->>>"+strsql1);                
				 ResultSet rs1= stmt.executeQuery(strsql1);     
				 while(rs1.next()){
				 ...
				 String inssql="insert into sk_srvcontrsch(rdocno, srno, brhid, delid, date, time, days) values('"+sccnttrno+"', '"+sccntsrno+"', '"+brhid+"', 0, '"+sqldate+"', '"+sctime+"', '"+days+"')";      
		    	 //System.out.println("inssql--->>>"+inssql);      
				 val= stmt.executeUpdate(inssql);              
	    	 ...
	    	 String inssql="insert into sk_srvcontrsch(rdocno, srno, brhid, delid, date, pskipno, status, time, days) values('"+cnttrno+"', '"+srno+"', '"+brhid+"', '"+delid+"', '"+sqldate+"', '"+poskipno+"', 2, '"+potime+"', '"+days+"')";                          
	    	 //System.out.println("inssql--->>>"+inssql);                
			 val= stmt.executeUpdate(inssql);     
			 
			 if(val>0){
					 String inssql1="insert into sk_srvcontrrep(delid, cnttrno, detsrno, oldskipno, newskipno, date, time, entrydatetime, userid, typeid, remarks) values('"+delid+"', '"+cnttrno+"','"+srno+"','"+poskipno+"','"+poskipno+"','"+sqldate+"','"+potime+"',now(),'"+session.getAttribute("USERID").toString()+"', 'P','"+poremarks+"')";                          
			    	 //System.out.println("inssql1--->>>"+inssql1);                       
					 val= stmt.executeUpdate(inssql1);      
					 
				     String logsql="insert into gl_biblog(doc_no, brhId, dtype, edate, userId, ENTRY, remarks) values ('"+docno+"','"+brhid+"','BSS',now(),'"+session.getAttribute("USERID").toString()+"','A', 'Pull Out')";  
	    	 ...
	    	 String inssql="insert into sk_srvcontrrep(delid, cnttrno, detsrno, oldskipno, newskipno, date, time, entrydatetime, userid, typeid, remarks) values('"+delid+"', '"+cnttrno+"','"+srno+"','"+oldskipid+"','"+poskipno+"','"+sqldate+"','"+potime+"',now(),'"+session.getAttribute("USERID").toString()+"', 'R','"+poremarks+"')";                       
	    	 //System.out.println("inssql--->>>"+inssql);                  
			 val= stmt.executeUpdate(inssql);     
			 if(val>0){
				 String upsql1="update sk_skipm set skipstatus=1 where doc_no='"+oldskipid+"'";                         
		    	// System.out.println("upsql1--->>>"+upsql1);                  
				 val= stmt.executeUpdate(upsql1);  
				 String upsql2="update sk_skipm set skipstatus=2 where doc_no='"+poskipno+"'";                            
		    	 //System.out.println("upsql2--->>>"+upsql2);                  
			 ...
			 if(val>0){
				 String upsql2="update sk_srvcontrdel set skipid='"+poskipno+"' where doc_no='"+delid+"'";                                  
		    	 //System.out.println("upsql2--->>>"+upsql2);                  
				 val= stmt.executeUpdate(upsql2);
				 
				 String logsql="insert into gl_biblog(doc_no, brhId, dtype, edate, userId, ENTRY, remarks) values ('"+docno+"','"+brhid+"','BSS',now(),'"+session.getAttribute("USERID").toString()+"','A', 'Replacement')";    
				 //System.out.println("logsql--->>>"+logsql);    
				 stmt.executeUpdate(logsql);             
			 }
			 if(val>0){
