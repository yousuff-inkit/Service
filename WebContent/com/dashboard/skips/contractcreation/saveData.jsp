		...
		if(processid.equalsIgnoreCase("Survery Update")){    
			...
			// System.out.println("delsql--->>>"+delsql);
			 ...
			 for(int i=0;i< griddataarray.size();i++){  
				 //System.out.println("Array size:"+griddataarray.get(i));
					...
					if(!(contrtypedet[0].trim().equalsIgnoreCase("undefined")|| contrtypedet[0].trim().equalsIgnoreCase("NaN")||contrtypedet[0].trim().equalsIgnoreCase("")|| contrtypedet[0].isEmpty())){
						...
						//System.out.println("chklistupdate--->>>"+upsql);  
						val= stmt.executeUpdate(upsql);
					}
			 }   
			 if(val>0) {   
				 String upsql1="update gl_enqm set presalesstatus=3 where doc_no='"+docno+"'";             
				 //System.out.println("upsql1--->>>"+upsql1);   
				 val= stmt.executeUpdate(upsql1);
	     ...
	     }else if(processid.equalsIgnoreCase("Return")){
	    	 java.sql.Date sqlDate=null;
			 if(!(date.equalsIgnoreCase("undefined"))&&!(date.equalsIgnoreCase(""))&&!(date.equalsIgnoreCase("0"))){
				     sqlDate=ClsCommon.changeStringtoSqlDate(date);
			 }  
			    
			 String upsql="update gl_enqm set returned=1,presalesstatus=6,returnremk='"+remarks+"' where doc_no='"+docno+"'";                       
			 //System.out.println("upsql--->>>"+upsql);
			 val= stmt.executeUpdate(upsql);
			 if(val>0){ 
				 sql="insert into gl_bqot( date, rdocno, rdtype, fdate, remarks, brhid, userid) values( now(), "+docno+", 'ENQ', '"+sqlDate+"', '"+remarks+"', "+brhid+", "+session.getAttribute("USERID").toString()+")";  
				 //System.out.println("followup=="+sql);
				 val= stmt.executeUpdate(sql);
				 
				 String logsql="insert into sk_presaleslog(doc_no, brhid, userid, description, entrydate) values ('"+docno+"','"+brhid+"','"+session.getAttribute("USERID").toString()+"','Returned to presales',now())";
				 //System.out.println("logsql--->>>"+logsql);  
				 val= stmt.executeUpdate(logsql);         
			 }
			 if(val>0){
