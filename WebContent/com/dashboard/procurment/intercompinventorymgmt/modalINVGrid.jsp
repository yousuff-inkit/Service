...
<script type="text/javascript">
	$(document).ready(function () {
   		...
   		var modalinvdata=[];
   		if(id=='1'){
   			modalinvdata='<%=DAO.getModalINVData(cldocno, detrowno, brhid, locid, date, billtype,vattype, id)%>';
		}
    ...
    // prepare the data
    	...
    	$("#modalINVGrid").on("bindingcomplete", function (event) {
    		...
    		for(var i=0;i<rows.length;i++){
    			if(rows[i].psrno!=null && rows[i].psrno!="undefined" && rows[i].psrno!="" && typeof(rows[i].psrno)!="undefined"){
    				...
    				$('#modalINVGrid').jqxGrid('setcellvalue',i,'taxpercent',taxpercent);
    				$('#modalINVGrid').jqxGrid('setcellvalue',i,'taxamount',taxamt);
    				$('#modalINVGrid').jqxGrid('setcellvalue',i,'taxtotal',taxtotal);
    			}
    		}
    	});
    	...
    	$("#modalINVGrid").jqxGrid(
    	{
        	...
        	columns: [
            	...
            	{ text: 'Allow Discount',datafield:'allowdiscount',width:'5%',hidden:true},
    	...
    	$("#modalINVGrid").on('cellendedit', function (event){
    		
    		var dataField = event.args.datafield;
    		var rowBoundIndex = event.args.rowindex;
    		var value = args.value;
    		funmodalINVGridChange(dataField,rowBoundIndex,value);
			
    	});
	});
	
	function funmodalINVGridChange(datafield,rowindex,value){
		var qty=$('#modalINVGrid').jqxGrid('getcellvalue',rowindex,'qty');
		var unitprice=$('#modalINVGrid').jqxGrid('getcellvalue',rowindex,'unitprice');
		var discountpercent=$('#modalINVGrid').jqxGrid('getcellvalue',rowindex,'discountpercent');
		var discountvalue=$('#modalINVGrid').jqxGrid('getcellvalue',rowindex,'discountvalue');
		var taxpercent=$('#modalINVGrid').jqxGrid('getcellvalue',rowindex,'taxpercent');
		...
		$('#modalINVGrid').jqxGrid('setcellvalue',rowindex,'total',total);
		$('#modalINVGrid').jqxGrid('setcellvalue',rowindex,'discountpercent',discountpercent);
		$('#modalINVGrid').jqxGrid('setcellvalue',rowindex,'discountvalue',discountvalue);
		$('#modalINVGrid').jqxGrid('setcellvalue',rowindex,'nettotal',subtotal);
		$('#modalINVGrid').jqxGrid('setcellvalue',rowindex,'taxamount',taxvalue);
		$('#modalINVGrid').jqxGrid('setcellvalue',rowindex,'taxtotal',taxtotal);
	}
</script>
<div id="modalINVGrid"></div>