<%@page import="net.sf.json.JSONArray"%>
<%@page import="net.sf.json.JSONObject"%>
<%@page import="com.connection.*"%>
<%@page import="java.sql.*"%>
<%
	JSONObject objdata = new JSONObject();
ClsConnection objconn = new ClsConnection();
Connection conn = null;
try {
	conn = objconn.getMyConnection();
	Statement stmt = conn.createStatement();
	String strgetbranch = " select b.branchname refname,b.doc_no docno,u.permission from my_brch b  left join my_user u on u.doc_no='"
	+ session.getAttribute("USERID") + "' "
	+ " left join my_usrbr ub on ub.user_id=u.doc_no and ub.brhid=b.doc_no and u.permission=1 "
	+ " where b.cmpid='" + session.getAttribute("COMPANYID") + "' and  if(u.permission=1,ub.user_id,'"
	+ session.getAttribute("USERID") + "')='" + session.getAttribute("USERID") + "'  and  b.status<>7";
	ResultSet rsgetbranch = stmt.executeQuery(strgetbranch);
	JSONArray brancharray = new JSONArray();
	int cnt = 0;
	while (rsgetbranch.next()) {
		JSONObject objtemp = new JSONObject();
		if (cnt == 0 && rsgetbranch.getString("permission").equalsIgnoreCase("0")) {
			objtemp.put("refname", "All");
			objtemp.put("docno", "");
			brancharray.add(objtemp);
			cnt = 1;
		}
		objtemp = new JSONObject();
		objtemp.put("refname", rsgetbranch.getString("refname"));
		objtemp.put("docno", rsgetbranch.getString("docno"));
		brancharray.add(objtemp);
	}

	String strbilltype = "select pg.billtype code,pg.sr_no doc_no from my_billtype pg where pg.status=3 and pg.type='PUR' order by seqno;";
	ResultSet rsgetbilltype = stmt.executeQuery(strbilltype);
	JSONArray billtypearray = new JSONArray();
	while (rsgetbilltype.next()) {
		JSONObject objtemp = new JSONObject();
		objtemp.put("code", rsgetbilltype.getString("code"));
		objtemp.put("docno", rsgetbilltype.getString("doc_no"));
		billtypearray.add(objtemp);
	}

	String strloc = "select doc_no,loc_name,address,tel from my_locm m";
	ResultSet rsgetloc = stmt.executeQuery(strloc);
	JSONArray locarray = new JSONArray();
	while (rsgetloc.next()) {
		JSONObject objtemp = new JSONObject();
		objtemp.put("location", rsgetloc.getString("loc_name"));
		objtemp.put("docno", rsgetloc.getString("doc_no"));
		locarray.add(objtemp);
	}
	
	
	objdata.put("branchdata", brancharray);
	objdata.put("billtypedata", billtypearray);
	objdata.put("locationdata",locarray);
	
} catch (Exception e) {
	e.printStackTrace();
} finally {
	conn.close();
}
response.getWriter().write(objdata + "");
%>