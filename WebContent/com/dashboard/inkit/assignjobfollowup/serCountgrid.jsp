...
%>
 ...
 String jobid =request.getParameter("jobid")==null?"0":request.getParameter("jobid").toString();
 String c_dtype =request.getParameter("c_dtype")==null?"0":request.getParameter("c_dtype").toString();
 String chktodate =request.getParameter("chktodate")==null?"0":request.getParameter("chktodate").toString();
...
var schedata; 
		 schedata= '<%=sd.serCountgrid(session,date,branch,clientid,id,priority,grp,emp,mem,area,chkfromdate,fromdate,jobid,c_dtype,chktodate)%>'; 
    ...
    $('#jqxSerCount').on('rowdoubleclick', function (event) 
    		{ 
    		   ...
    		   var isprior=document.getElementById("isprior").value;
    		     ...
    		     document.getElementById("dtyp").value=dtype;
    		   
    		$("#overlay, #PleaseWait").show();
    		var id=1;
    		//console.log("serScheduleDetails.jsp?clientid="+clientid+"&date="+date+"&barchval="+barchval+"&dtype="+dtype+"&id="+id+"&isprior="+isprior+"&grp="+grp+"&emp="+emp+"&mem="+mem+"&area="+area+"&bttnview="+bttnview+"&chkfromdate="+'<%=chkfromdate%>'+"&fromdate="+'<%=fromdate%>');
    		$("#serschedulediv").load("serScheduleDetails.jsp?clientid="+clientid+"&date="+date+"&barchval="+barchval+"&dtype="+dtype+"&id="+id+"&isprior="+isprior+"&grp="+grp+"&emp="+emp+"&mem="+mem+"&area="+area+"&bttnview="+bttnview+"&chkfromdate="+'<%=chkfromdate%>'+"&fromdate="+'<%=fromdate%>'+"&jobid="+'<%=jobid%>'+"&chktodate="+'<%=chktodate%>');
