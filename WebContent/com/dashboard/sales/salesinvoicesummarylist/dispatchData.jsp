...
for(int i=0;i<invarray.size();i++){
	...
	String strsql="update my_invm set dispatch=1 where doc_no="+invarray.get(i);
	int val=stmt.executeUpdate(strsql);
	if(val>0){
			Statement stmtbranch=conn.createStatement();
			ResultSet rsbranch=stmtbranch.executeQuery("select brhid from my_invm where doc_no="+invarray.get(i)+"");
			int branch=0;
			if(rsbranch.next()){
				branch=rsbranch.getInt("brhid");	
			}
			Statement stmtlog=conn.createStatement();		
		    String upsql="insert into gl_biblog (doc_no, brhId, dtype, edate, userId, ENTRY) values ("+invarray.get(i)+",'"+branch+"','BIIL',now(),'"+session.getAttribute("USERID").toString()+"','E')";
