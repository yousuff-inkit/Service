	...
	int docval=0;
	   ...
	   if(cmbText.equalsIgnoreCase("Follow-Up"))
	   {
		 ...
		 /*   upsql="update  my_prfqd  set clstatus=1 where doc_no='"+docno+"' ";
		     ...
		     upsql="select coalesce(max(doc_no)+1,1) doc_no from my_bsef";
			   ResultSet resultSet = stmt.executeQuery(upsql);
			   
			    if (resultSet.next()) {
			    	docval=resultSet.getInt("doc_no");
			    }	 
		   upsql="insert into my_bsef(doc_no,date, rdocno, bibpid, fdate, remarks, userid, status,prdid)values('"+docval+"',now(),'"+docno+"','"+cmbinfo+"','"+sqlprocessdate+"','"+remarks+"','"+session.getAttribute("USERID").toString()+"',3,'"+prodoc+"') ";
		  // System.out.println("====4====="+upsql);
			 val= stmt.executeUpdate(upsql);
			 upsql="insert into gl_biblog (doc_no, brhId, dtype, edate, userId, ENTRY) values ('"+docval+"','"+branchids+"','BSEF',now(),'"+session.getAttribute("USERID").toString()+"','A')";
	   ...
	   else  if(cmbText.equalsIgnoreCase("Dropped"))
	   {
		     ...
		     upsql="insert into my_bsef (doc_no,date, rdocno, bibpid, ddate, remarks, userid, status,prdid)values('"+docval+"',now(),'"+docno+"','"+cmbinfo+"','"+sqlprocessdate+"','"+remarks+"','"+session.getAttribute("USERID").toString()+"',3,'"+prodoc+"') ";
			   System.out.println("====4====="+upsql);
			 val= stmt.executeUpdate(upsql);
			 
			  upsql="update  my_cusenqm  set status=6 where doc_no='"+docno+"'  ";
