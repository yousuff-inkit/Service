...
$(document).ready(function () {
   ...
   if(enqtrno>0){
	   ...
	   estvariationdata='<%=DAO.materialGridExcel(session,enqtrno,ecmno)%>';
   }
   else{
	   matdata=[];   
   }
            ...
            $("#materialEstPriceGrid").jqxGrid(
            {
              ...
              //  disabled:true,
                ...
                columns: [
							...
							{ text: 'Description', datafield: 'desc1', width: '20%',editable:true },
...
{ text: 'Brand', datafield: 'brandname', width: '12%'  ,cellclassname: cellclassname ,columntype: 'custom',
							...
							{ text: 'Quantity', datafield: 'qty', width: '5%',editable:false,cellclassname: cellclassname, cellsformat: 'd2', cellsalign: 'right', align: 'right' },
							{ text: 'Variant Quantity', datafield: 'newqty', width: '8%',cellclassname: cellclassname, cellsformat: 'd2', cellsalign: 'right', align: 'right' },
							...
							{text: 'balqty', datafield: 'balqty', width: '10%',cellclassname: cellclassname,hidden:true},
            ...
            $("#materialEstPriceGrid").on('cellvaluechanged', function (event) 
    		    ...
    		    var amount=0;
    		    var nettotal=0;
    		   var  newqty=0; parseFloat($('#materialEstPriceGrid').jqxGrid('getcellvalue', rowBoundIndex, "newqty"));	
              	var balqty=0; parseFloat($('#materialEstPriceGrid').jqxGrid('getcellvalue', rowBoundIndex, "balqty"));
    		    ...
    		    if(datafield=="qty" || datafield=="amount" || datafield=='margin' )
    		  {
    		    ...
    		    if(datafield == "newqty")
    			  { 
    		    	qty=parseFloat($('#materialEstPriceGrid').jqxGrid('getcellvalue', rowBoundIndex, "qty"));	
    		    	 newqty=parseFloat($('#materialEstPriceGrid').jqxGrid('getcellvalue', rowBoundIndex, "newqty"));	
    	              	balqty=parseFloat($('#materialEstPriceGrid').jqxGrid('getcellvalue', rowBoundIndex, "balqty"));
                   	var valqty=newqty+balqty;
    		    	if(valqty<0)
    		    		{
    		    		
    		    	  	 $.messager.alert('Message', 'New Quantity Mismatch, Balance Quantity Is '+balqty);
    		    		 $("#materialEstPriceGrid").jqxGrid('setcellvalue', rowBoundIndex, "newqty", 0.00);
    		    		}
