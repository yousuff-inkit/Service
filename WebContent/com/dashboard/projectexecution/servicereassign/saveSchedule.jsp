...
<% 
 ...
 String srtrno =request.getParameter("srtrno")==null?"":request.getParameter("srtrno").toString();
	System.out.println("srtrno=="+srtrno);	 
...
ClsCommon ClsCommon=new ClsCommon();
	...
	try{
		...
		int rstatus=0;
		  ...
		  String consql="select restatus from cm_servplan where TR_NO="+srtrno+"";
		  System.out.println("consql==="+consql);
		  ResultSet rsconfg = stmt.executeQuery(consql);

		  while (rsconfg.next()) {
			  rstatus=rsconfg.getInt("restatus");
		  
		  }
		rstatus=rstatus+1;
			 strSql = "update cm_servplan set empGroupId="+groupid+", empId="+grpempid+", asgnMode="+assignid+" ,remarks='"+desc+"',plannedon='"+sdate+"',date='"+sdate+"',pltime='"+ptime+"',workper=0,status=4,restatus="+rstatus+" where TR_NO="+srno+"  and dTYPE='"+dtype+"'  "+sql1+"";      
		
		int rs = stmt.executeUpdate(strSql);
		if(dtype.equalsIgnoreCase("CREG")){
		 strSqlcreg = "update cm_cuscallm set clstatus=0 where tr_no="+trno+""; 
		 System.out.println("strSqlcreg==="+strSqlcreg);
		int rs1 = stmt.executeUpdate(strSqlcreg);
		}
	System.out.println(rs);
		if(rs>0){
			
			
			 String sqlassign="insert into gl_bsra (brhid,refDocNo, empGroupId, empId, asgnMode, plannedOn, plTime, description) values ('"+branchid+"','"+srtrno+"',"+groupid+","+grpempid+","+assignid+",'"+sdate+"','"+ptime+"','"+desc+"')";
		 System.out.println("==sqlassign===="+sqlassign);
			 stmt.executeUpdate(sqlassign);
			
			 
			 
			 String sqls="insert into gl_biblog (doc_no, brhId, dtype, edate, userId, ENTRY) values ('"+srtrno+"','"+branchid+"','BSRA',now(),'"+userid+"','E')";
