...
try{
	...
	String strgetmsg="select usr.user_name,job.msg,date_format(job.msgdate,'%d.%m.%Y %H:%i') msgdate from rl_comments job left join my_user usr on job.userid=usr.doc_no where job.status=3 and job.dtype='CGC' and job.doc_no="+docno+"  order by rowno desc";
