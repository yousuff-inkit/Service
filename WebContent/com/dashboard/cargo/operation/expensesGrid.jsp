...
$(document).ready(function () {
	...
	var source =
	{
            ...
            funCalcBalance(); 
            $("#jqxExpense").jqxGrid('addrow', null, {});
            $("#jqxExpense").jqxGrid({disabled:true}); 
            $("#jqxExpense").on("celldoubleclick", function (event){
            	var columnindex1=event.args.datafield;
            	var rowindex=event.args.rowindex;   
            	$('#rowindex2').val(rowindex);     
            	if(columnindex1 == "sertype")      
            	{ 
            		serTypeSearchContent("serviceTypeSearch.jsp?modeid="+$("#hidmodeid").val()+"&shipid="+$("#hidshipid").val()+"&smodeid="+$("#hidsmodeid").val()+"&id="+2);                
            	}
            	if(columnindex1 == "account")            
            	{ 
            		var type= $('#jqxExpense').jqxGrid('getcellvalue', rowindex, "type"); 
            		var atype="";
            		if(type=="CREDIT" || type=="CASH-VAT"){      
            			 atype="AP";
            			 accountSearchContent("accountSearchGrid.jsp?atype="+atype);      
            		}else if(type=="HR"){
            			 atype="HR";            
            			 accountSearchContent("accountSearchGrid.jsp?atype="+atype);      
            		}else{}     
          ...
          $("#jqxExpense").on('cellvaluechanged', function (event) 
            {
            	var datafield = event.args.datafield;
            	var rowBoundIndex = event.args.rowindex;           
            	if(datafield=="amount" || datafield=="vatper"){                       
            		var amount= $('#jqxExpense').jqxGrid('getcellvalue', rowBoundIndex, "amount");
            		var vatper= $('#jqxExpense').jqxGrid('getcellvalue', rowBoundIndex, "vatper");
            		var vatamt=(parseFloat(amount)*parseFloat(vatper))/100;
            		var nettotal=parseFloat(amount)+vatamt;       
            		$("#jqxExpense").jqxGrid('setcellvalue', rowBoundIndex, "vatamt", vatamt);
            		$("#jqxExpense").jqxGrid('setcellvalue', rowBoundIndex, "nettotal", nettotal);  
