...
try {
	...
	String sql = "select date_format(d.edate,'%d.%m.%Y %h:%i')  edate,case when d.entry='A' then 'ADD' when d.entry='E' then 'EDIT'  when d.entry='D' then 'DELETE' else '' END  entry,u.user_name from sk_srvqotm m left join (select max(trno) trno,doc_no,brhid,dtype,edate,entry from datalog group by doc_no,brhid,dtype) d on d.doc_no=m.doc_no   and d.brhid=m.brhid and d.dtype='QOT' left join datalog dd on dd.trno=d.trno left join my_user u on u.doc_no=m.userid where m.tr_no='"+id+"'";
