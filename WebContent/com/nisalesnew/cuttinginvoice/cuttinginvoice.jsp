	  ...
	  salesmanSearchContent('SearchSalesman.jsp?', $('#Salesmanwindow')); 
	   ...
	   $('#itemdocno').dblclick(function(){
		 ...
		 $('#rrefno').dblclick(function(){
			 ...
			 refsearchContent1('dlymainsearch.jsp');
...
function getsal(event){
 ...
 function funNotify(){
	 ...
	 var rows = $("#prdGridID").jqxGrid('getrows');
	   ...
	   newTextBox.appendTo('form');
		...
		document.getElementById("totalamount").value=document.getElementById("totalamount").value.replace(/\,/g,"");
		
		   if (document.getElementById("charge").value.indexOf(',') > -1)
		   {
			   document.getElementById("charge").value=document.getElementById("charge").value.replace(/\,/g,"");
		   }
		
		
		   if (document.getElementById("discount").value.indexOf(',') > -1)
		   {
			   document.getElementById("discount").value=document.getElementById("discount").value.replace(/\,/g,"");
		   }
		
		document.getElementById("nettotal").value=document.getElementById("nettotal").value.replace(/\,/g,"");
		   if (document.getElementById("taxamount").value.indexOf(',') > -1)
		   {
		document.getElementById("taxamount").value=document.getElementById("taxamount").value.replace(/\,/g,"");
		   }
		document.getElementById("invvalue").value=document.getElementById("invvalue").value.replace(/\,/g,"");  
 ...
 function calculatedata()
 {
	...
	if(price=="" || price==null || price==0 || typeof(price)=="undefined"|| typeof(price)=="NaN")
	{
	...
	else
		{
			...
			document.getElementById("amounts").value=(parseFloat(price)*parseFloat(quantity)).toFixed(4);
 ...
 function changedtype()
 {
	...
	function funRoundAmt1(value ,id){
...
function calculatedatanet()
{
	 
    var totalamount= document.getElementById("totalamount").value.replace(/\,/g,"");
	var charge= document.getElementById("charge").value.replace(/\,/g,"");
	var discount= document.getElementById("discount").value.replace(/\,/g,"");
 	var taxamount= document.getElementById("taxamount").value.replace(/\,/g,"");
	var taxper=$('#hidtaxper').val();
	
	if(totalamount=="" || totalamount==null || totalamount==0 || typeof(totalamount)=="undefined"|| typeof(totalamount)=="NaN")
	{totalamount=0;}
	if(charge=="" || charge==null || charge==0 || typeof(charge)=="undefined"|| typeof(charge)=="NaN")
	{charge=0;}
	if(discount=="" || discount==null || discount==0 || typeof(discount)=="undefined"|| typeof(discount)=="NaN")
	{discount=0;}
	if(taxamount=="" || taxamount==null || taxamount==0 || typeof(taxamount)=="undefined"|| typeof(taxamount)=="NaN")
	{taxamount=0;}
	 var totval=addCommasval(parseFloat(totalamount).toFixed(4));
	 document.getElementById("totalamount").value=totval;
	 var chg=addCommasval(parseFloat(charge).toFixed(4));
	 document.getElementById("charge").value=chg;
	 var discountval=addCommasval(parseFloat(discount).toFixed(4));
	 document.getElementById("discount").value=discountval;
	 var calutionamount=(parseFloat(totalamount)+parseFloat(charge))-parseFloat(discount);
	 var nettoatl=(parseFloat(calutionamount).toFixed(4));
	 var nettoatll=addCommasval(parseFloat(calutionamount).toFixed(4));
	 document.getElementById("nettotal").value=nettoatll;
	 var netvalcal=parseFloat(calutionamount)*parseFloat(taxper/100);
	 var taxamountval=(parseFloat(netvalcal).toFixed(4));
	 var taxamountvall=addCommasval(parseFloat(netvalcal).toFixed(4));
	 document.getElementById("taxamount").value=taxamountvall;
	 var invtotal=parseFloat(taxamountval)+parseFloat(nettoatl);
	 var netval=addCommasval(parseFloat(invtotal).toFixed(4));
	 document.getElementById("invvalue").value=netval;
	   ...
	   if (($("#mode").val() == "view") && $("#masterdoc_no").val()!="") {
		  ...
		  PrintContent('printVoucherWindow.jsp');
	    	 ...
	    	 var brhid=<%=session.getAttribute("BRANCHID").toString()%>
	    	    	 ...
	    	    	 $.messager.confirm('Message', 'Do you want to include logo?', function(r){
		    	           
		    	         if(r==false)
		    	           {
		    	        	 
		    	        	 var win= window.open(reurl[0]+"printcutinv?docno="+document.getElementById("masterdoc_no").value+"&dtype="+dtype+"&brhid="+brhid+"&type=yes"+"&logo=0","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");    
		    		    	 win.focus(); 
		    	           }
		    	         else{
		    	        	 
		    	        	 var win= window.open(reurl[0]+"printcutinv?docno="+document.getElementById("masterdoc_no").value+"&dtype="+dtype+"&brhid="+brhid+"&type=yes"+"&logo=1","_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");    	        	 
		    	        	 win.focus();
		    	         }
		    	    });
...
<form id="frmsave" action="saveinvs1" autocomplete="OFF" method="post" > 
...
<table width="100%"   >
  <tr>
    ...
    <td width="25%" align="right">RefType<select id="rreftype" name="rreftype" width="10%" onchange="changedtype()" ><option>DIR</option><option>DLY</option> </select>
 ...
 <tr>
  ...
  <td align="right" width="5%">Del. Date</td>
    <td width="8%"><div id='deliveryDate' name='deliveryDate' value='<s:property value="deliveryDate"/>'></div></td>
     ...
     <td align="right" width="10%" >Type<select id="ftype"  style="width:75%;"  name="ftype" >
    <option value="3">Credit</option>
     ...
     Remarks <input name="remarks" type="text"  style="width:90%;" id="remarks" value='<s:property value="remarks"/>' ></td> 
...
<td>   <input type="text" id="amounts" tabindex="-1"    onkeypress="javascript:return isNumber1 (event);"  style="text-align: right;" ></td>
  ...
  <input type="button" id="setbtn"  class="btn" onclick="setgrid()" value="ADD" ></td>
    ...
    <select hidden="true"  type="text" id="pcode"   > 
   <input type="hidden" id="prate"   onblur="funRoundAmt(this.value,this.id);" onkeypress="javascript:return isNumber1 (event);"  onchange="calculatedata();" style="text-align: right;" > 
    <input type="hidden" id="pcharge" tabindex="-1" onblur="funRoundAmt(this.value,this.id);" onkeypress="javascript:return isNumber1 (event);"  style="text-align: right;"   > 
  <input type="hidden" id="temp"   > 
    <select hidden="true"  type="hidden" id="bcode"   ></select>
     <input type="hidden" id="floor"    > 
  <input type="hidden" id="flat"   > 
  <select hidden="true" id="sqm" onchange="calculatedata();"><option></option><option>0</option><option>0</option></select>
 
    <input type="hidden" id="totsize" tabindex="-1"   onkeypress="javascript:return isNumber1 (event);"  style="text-align: right;"   > 
   <input type="hidden" id="brate"   onblur="funRoundAmt(this.value,this.id);" onkeypress="javascript:return isNumber1 (event);"  onchange="calculatedata();"  style="text-align: right;"  >
      <input type="hidden" id="bvalue" tabindex="-1" onblur="funRoundAmt(this.value,this.id);" onkeypress="javascript:return isNumber1 (event);"    style="text-align: right;"  > 
   </table> 
   </fieldset> 
...
<table width="100%">
   ...
   <tr>  
    ...
    <td align="left" width="10%"> <input type="text" id="totalamount" name="totalamount"  value='<s:property value="totalamount"/>'     onchange="calculatedatanet();" style="text-align: right;"></td>
  <td align="right"  width="5%">Charge</td>
 
   <td align="left"  width="10%"> <input type="text" id="charge" name="charge"   value='<s:property value="charge"/>'   onkeypress="javascript:return isNumber1 (event);"  onchange="calculatedatanet();" style="text-align: right;"></td>
   <td align="right"  width="5%">Discount</td>
   <td align="left"  width="10%"> <input type="text" id="discount" name="discount"   value='<s:property value="discount"/>'    onkeypress="javascript:return isNumber1 (event);"  onchange="calculatedatanet();" style="text-align: right;"></td>
 <td align="right"  width="6%">Net Total</td>
   <td align="left"  width="10%"> <input type="text" id="nettotal"  name="nettotal"   value='<s:property value="nettotal"/>'    onchange="calculatedatanet();" style="text-align: right;"></td>
 <td   width="6%" align="right">Tax Amount</td>
   <td   width="10%" align="left"> <input type="text" id="taxamount" name="taxamount"   value='<s:property value="taxamount"/>'    onkeypress="javascript:return isNumber1 (event);"  onchange="calculatedatanet();" style="text-align: right;"></td>
  <td  width="6%" align="right">Invoice Vlaue</td> 
   <td  width="10%" align="left"> <input type="text" id="invvalue"  name="invvalue"   value='<s:property value="invvalue"/>'   onchange="calculatedatanet();" style="text-align: right;"></td>
 ...
 <input type="hidden" id="hidftype" name="hidftype"  value='<s:property value="hidftype"/>'/>
...
<input type="hidden" name="deleted" id="deleted" value='<s:property value="deleted"/>' />
  ...
  <input type="hidden" id="cplogo" name="cplogo"  value='<s:property value="cplogo"/>'/>
  <input type="hidden" id="hidtaxper" name="hidtaxper"  value='<s:property value="hidtaxper"/>'/>
</div>
</form>
 <div id="searchwindow">
   <div ></div>
