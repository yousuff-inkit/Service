...
function playsound(){
    ...
    audioElement.setAttribute('src','<%=contextPath+"/"%>icons/sms.mp3');
...
function getMsgHistory(){
	...
	//alert("==usertoid==="+usertoid);
	//alert("==userfrmid==="+userfrmid);
	...
	if (x.readyState==4 && x.status==200)
		{
			...
			user=items[0];
			msgcnt=items[1];
			document.getElementById("msgcnt").value=msgcnt;
				if(msgcnt>0){
					playsound();
...
x.send();
 ...
 function usergridload(){
	 ...
	 var compid=document.getElementById("compid").value.trim();
	 var msgcnt=document.getElementById("msgcnt").value.trim();
	 $('#UserGrid').load('usersearch.jsp?comp_id='+compid+'&msgcnt='+msgcnt);
 }
                ...
                $("#jqxWidget").on('select', function (event) {
                    if (event.args) {
                        ...
                        if (item) {
                            ...
                            document.getElementById("usertoid").value=0;
                            var userfrmid=document.getElementById("userfrmid").value.trim();
                            var usertoid=document.getElementById("usertoid").value.trim();
...
<table width="100%">
  ...
  <tr>
  <td><textarea id="txtmsgenter" style="height:65px;width:100%;font: 10px Tahoma;resize:none"  placeholder="Enter message..." name="txtmsgenter" onkeydown="sendMsgenter(event);"><s:property value="txtmsgenter" ></s:property></textarea></td>
  </tr>
</table>
<input id="width" type="hidden" value="640" />
    ...
    <input type="hidden" id="compid" name="compid"  value='<s:property value="compid"/>'/>
    <input type="hidden" id="compcode" name="compcode"  value='<s:property value="compcode"/>'/>
    <input type="hidden" id="msgcnt" name="msgcnt"  value='<s:property value="msgcnt"/>'/></td>
    </td>
  </tr>
</table>
