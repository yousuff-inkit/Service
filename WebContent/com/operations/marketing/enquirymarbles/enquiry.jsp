...
var brhid='<%=brhid%>';
   ...
   if(document.getElementById("txtradio").value=="")
	   ...
	   $("#cmbclient").hide();
		...
		//	document.getElementById("r1").checked = false;
			...
			$('#txtcat').attr('readonly', true );
			$('#txtsubcat').attr('readonly', true );
 ...
 function getcontact(event){
    ...
    function funReset(){
		//$('#frmEnquirymbl')[0].reset(); 
	}
	function funReadOnly(){
	;
		$('#frmEnquirymbl input').prop('readonly', true );
		
		$('#frmEnquirymbl textarea').attr('readonly', true );
		$('#frmEnquirymbl select').attr('disabled', true);
	...
	//	$('#r2').attr('disabled', true);
		...
		document.getElementById("mode").value=modes;
          ...
          document.getElementById("frmEnquirymbl").submit();
	...
	function funRemoveReadOnly(){
		...
		$('#frmEnquirymbl input').attr('readonly', false );
		$('#frmEnquirymbl textarea').attr('readonly', false );
		$('#frmEnquirymbl select').attr('disabled', false);
 ...
 function getsertype(rowBoundIndex)
 {
			       ...
			       document.getElementById("cmbclient").value=items[5];
		              document.getElementById("txtclientname").value=items[6];
			           ...
			           document.getElementById("txtsource").value=items[12];
			              ...
			              document.getElementById("txtcat").value=items[18];
			              document.getElementById("txtsubcat").value=items[19];
	...
	//site grid
		 ...
		 var rows = $("#jqxEnquiry").jqxGrid('getrows');
		   ...
		   newTextBox.val(rows[i].descritpion+" :: "+rows[i].qty+" :: "+rows[i].remarks+" :: "+rows[i].unitid+" :: ");
	...
	function reqdata()
	{
 		 /* if((document.getElementById("r1").checked=="")&&(document.getElementById("r2").checked==""))
    		...
    		{ 
     	      ...
     	      $('#frmEnquirymbl input').attr('readonly', true );
     			$('#frmEnquirymbl textarea').attr('readonly', true );
...
var win= window.open(reurl[0]+"printEnqmbl?docno="+document.getElementById("masterdoc_no").value,"_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
...
<form id="frmEnquirymbl" action="saveEnquirymbl" autocomplete="OFF" method="POST">      
...
<table width="100%"  >                        
  ...
  <tr>
    ...
    <td colspan="3"><input type="text" id="cmbclient" name="cmbclient" placeholder="Press F3 To Search" style="width: 19%;" value='<s:property value="cmbclient"/>' onKeyDown="getclinfo(event);" onfocus="disfields();">
		<input type="text" id="txtclientname" name="txtclientname" style="width: 54%;" value='<s:property value="txtclientname"/>' onfocus="reqdata();disfields();"></td>
  ...
  <td width="10.8%" align="right">Contact person</td>
    <td colspan="4"><input type="text" id="txtcontact" name="txtcontact" style="width:12%;" value='<s:property value="txtcontact"/>' onKeyDown="getcontact(event);">
     <label >Category</label><input type="text" id="txtcat" readonly name="txtcat" style="width:10%;" value='<s:property value="txtcat"/>'/>
      <label >SubCategory</label><input type="text" id="txtsubcat" readonly name="txtsubcat" style="width:15%;" value='<s:property value="txtsubcat"/>'/>
     Salesman
   <input type="text" id="txtsalesman" name="txtsalesman" style="width:25%;" placeholder="Press F3 To Search" value='<s:property value="txtsalesman"/>' onKeyDown="getsalinfo(event);" onfocus="disfields();">
   ...
   <tr>
    ...
    <td><input type="text" id="txtaddress" name="txtaddress" style="width:77%;" value='<s:property value="txtaddress"/>' onfocus="reqdata();disfields();"></td>
  </tr>
  </table>
<table  width="100%"  >
  <tr>
    <td align="right" width="10.8%">MOB</td>
    <td width="20%" ><input type="text" id="txtmobile" name="txtmobile" style="width:55%;" value='<s:property value="txtmobile"/>' onfocus="reqdata();disfields();"></td>
    ...
    <td > <input type="email" id="txtemail" name="txtemail" style="width: 52%;" value='<s:property value="txtemail"/>' onfocus="reqdata();disfields();"></td>
  </tr>
  ...
  <tr>
    ...
    <td align="right" hidden="true">Type</td>
	  <td align="left" hidden="true"><select name="cmbprocess" id="cmbprocess" style="width:62%;" name="cmbprocess" onchange="funtxtenable(value);"  value='<s:property value="cmbprocess"/>'></select>
	  <input type="hidden" name="hidcmbprocess" id="hidcmbprocess" value='<s:property value="hidcmbprocess"/>' /> 
	  </td>
	  
    <td align="right" >Source</td>
  <td align="left" width="20%"><input type="text" id="txtsource" name="txtsource" style="width:60%;" readonly placeholder="Press F3 To Search" onKeyDown="getSource(event);" value='<s:property value="txtsource"/>'></td>
	<td hidden="true">  <input type="checkbox" name="chksurvey" id="chksurvey" value="chksurvey"  value='<s:property value="chksurvey"/>' onclick="$(this).attr('value', this.checked ? 1 : 0);">Surveyed</td>
  ...
  <td align="right" width="10.8%">Remarks</td> 
    <td colspan="3">
    <input type="text" id="txtRemarks" name="txtRemarks" style="width:77%;" value='<s:property value="txtRemarks"/>'onfocus="reqdata();disfields();"></td>
