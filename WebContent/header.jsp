	...
	function getformbranch(){
		...
		///alert($('#brchName').val());
		  ...
		  function JSONToCSVCon(JSONData, ReportTitle, ShowLabel) {
			...
			function dateformatchanger(name,date){            
				 var startdate=date.split('.');    
		         $('#'+name).jqxDateTimeInput('setDate',new Date(startdate[2],startdate[1]-1,startdate[0]));            
			}
			...
			function checkReftype(){
				...
				//alert("test : "+test)
								   ...
								   if (x.readyState==4 && x.status==200)
								    {
								      var items= x.responseText.trim();
									   ...
									   var optionspg='<option value="DIR">DIR</option>';
									   
									   $.each(jsonObject.refdata,function(index,value){
										   if(jsonObject.refdata=='0'){
											   //$('.refid').show();
											   $('.refid').style.display='block';
										   }else{
											   //$('.refid').style.display='none';
											   optionspg+= '<option value="' + value.reftype + '">' + value.reftype + '</option>';

										   }
																   
									   });
									   if(dtype=='DEL' || dtype=='INV'){
									   
										   $("select#cmbreftype").html(optionspg);
									   }else{
										   $("select#reftype").html(optionspg);
									   }
								   ...
								   x.open("GET",<%=contextPath+"/"%>+"chckreftype.jsp?dtype="+dtype,false);
									x.send();
							}
...
<div id="mainBG" class="homeContent" data-type="background">
    <div class="HeadIcons" id="full">
        ...
        <!-- Hidden fields strictly from your new script requirements -->
    ...
    <div class="action-bar">
        ...
        <button type="button" class="action-btn" id="btnSave" title="Save Changes" onclick="funSaveBtn()" style="display: none;">
