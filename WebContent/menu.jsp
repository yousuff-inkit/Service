			...
			if (x.readyState==4 && x.status==200)
				{
					...
					if(Data.split("$$$$$",1)!=""){
						  ...
						  //alert(aa+"=="+val);         
						  if(val=="1" && aa!="HR Related Master" && aa!="Veh. Related Master" && aa!="Maintenance Master" && aa!="Product Related Master" && aa!="Asset Related Master" && aa!="Client Area" && aa!="Company Settings" && aa!="Service Settings" && aa!="Staff Master"  && aa!="Tax Master "  && aa!="Transport Masters"  && aa!="Property Related Master" && aa!="Dump Related Master"  && aa!="Floor Master"  && aa!="Floor Master"){
...
function funClear(){
	...
	function funToggleCompDropdown(){
		document.getElementById("compdropdown").classList.toggle("show");
	}
	
	function getInterCompanies(){
		$.get('getMultiCompany.jsp',function(data){
			data=JSON.parse(data);
			var htmldata='';
			if(data.multicompconfig=="1"){
				$('.compdropdown').show();
			}
			else{
				$('.compdropdown').hide();
			...
			$.each(data.dbdata,function(index,value){
				htmldata+='<a href="#!" data-compdocno="'+value.compdocno+'">'+value.compname+'</a>';
			});
			
			$('#compdropdown').html($.parseHTML(htmldata));
			
			$('#compdropdown a').each(function(){
            	$(this).on('click',function(){
            		var compdocno=$(this).attr('data-compdocno');
            		location.href='logout?compdocno='+compdocno;  
            	});
            });
		});
	}
	
	function funToggleLinksDropdown(){
		document.getElementById("linkdropdown").classList.toggle("show");
	}

	function getMultiLinks(){
		$.get('getMultiLinks.jsp',function(data){
			//console.log("Server response:", data);
			data=JSON.parse(data);
			var htmldata='';
			if(data.multilinkconfig=="1"){
				$('.linkdropdown').show();
			}
			else{
				$('.linkdropdown').hide();
			}
			$.each(data.urldata,function(index,value){
				htmldata += '<a href="javascript:void(0);" data-link="'+value.link+'">'+value.name+'</a>';
			});
			
			$('#linkdropdown').html($.parseHTML(htmldata));
			$('#linkdropdown a').each(function() {
			    $(this).on('click', function() {
			        var name = $(this).text(); 
	...
	function viewLink(name, path) {
	    var url = document.URL; 
	    var reurl;

	    if (url.includes("login#")) {
	        reurl = url.split("login#");
	    } else {
	        reurl = url.split("login");
	    }

	   
	    funToggleLinksDropdown();   

	    
	    var msg = getUrlParameter('msg');
	    var mode = getUrlParameter('mode');

	    var newTabUrl = reurl[0] + path + (msg ? '&msg=' + msg : '') + (mode ? '&mode=' + mode : '');   
...
<!-- 		<button id="icon" name="approve"  title="Executive Folio" onclick="funApprove()">
    	...
    	<div class="dropdown compdropdown" hidden="true">
    		<button id="icon1" title="Switch Company" onclick="funToggleCompDropdown();">
				<img alt="Logout" src="<%=contextPath%>/icons/btnbookreload.png" style="width:24px;height:24px;">
			</button>
			<div id="compdropdown" class="dropdown-content">
    			
  			</div>
    	</div>
    	
    	<div class="dropdown linkdropdown" hidden="true">   
    		<button id="icon1" title="URLS" onclick="funToggleLinksDropdown();">
				<img alt="Logout" src="<%=contextPath%>/icons/icons-multilinks.png" style="width:24px;height:24px;">     
			</button>
