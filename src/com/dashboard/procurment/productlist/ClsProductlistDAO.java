...
public class ClsProductlistDAO { 
	...
	/*        java.sql.Date frmdate = null;
	    	...
	    	if(!((branchid.equalsIgnoreCase("a")) || (branchid.equalsIgnoreCase("NA")) || (branchid.equalsIgnoreCase("0")))){
	    		...
	    		sqls1=" and (de.brhid='"+branchid+"' or m.comp_brch='C')";
	 		}
		...
		if(type.equalsIgnoreCase("1"))
		{
			...
			sql="select @i:=@i+1 slno, a.* from(Select dep.department,sc.subcategory,pt.producttype type,cat.category cat,bd.brandname brand, at.mspecno as specid, m.part_no productid,"
					...
					+ "   "+sqls+" "+sqls1+" group by m.doc_no)a,(select @i:=0)c;";




	// System.out.println("==product list1===="+sql);
