package com.dashboard.procurment.stocklist1;
...
public class ClsStocklist1DAO {
	...
	public   JSONArray stocklist(String branch,String status,String psrno,String load) throws SQLException {
      ...
      /*  
		...
		try {
				...
				String sql=" select m.barcode,m.jdeno,m.bhno,c.brhid,bd.brandname,ca.category,c.qty,m.part_no productid,m.productname,c.amount,u.unit unit,sc.subcategory subcategory from (select  b.brhid,sum(b.qty) qty,b.prdid,sum(amount) amount from (select i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
	...
	public   JSONArray stockExcellist(String branch,String status,String psrno,String load) throws SQLException {
      ...
      /*  
		...
		try {
				...
				String sql=" select m.part_no 'product id',m.productname 'product Name',m.jdeno,m.bhno, bd.brandname 'Brand Name',c.qty,c.amount from "
	...
	public   JSONArray stockExcellistqty(String branch,String status,String psrno,String load) throws SQLException {
      ...
      /*  
		...
		try {
				...
				String sql=" select m.part_no 'product id',m.productname 'product Name', bd.brandname 'Brand Name',m.barcode 'Bar Code',m.jdeno,m.bhno,ca.category 'Category', sc.subcategory 'Subcategory',c.qty,u.unit 'Unit'  from "
						+ "(select  b.brhid,sum(b.qty) qty,b.prdid,sum(amount) amount from (select i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
						    + "  ,i.cost_price*(i.op_qty-coalesce(a.outqty,0)) amount from my_prddin i "
							+ "   left join (select sum(qty)+sum(del_qty)+sum(rsv_qty) outqty, stockid ,prdid from my_prddout group by stockid) a on a.stockid=i.stockid "
      ...
      /*  
		...
		try {
				...
				*/
				...
				String sql="  select m.barcode,m.jdeno,m.bhno,bc.branchname,lc.loc_name,c.locid,c.brhid,bd.brandname,ca.category,c.qty,m.part_no productid,m.productname,c.amount,sc.subcategory,u.unit from "
      ...
      /*  
		...
		try {
				...
				*/
				...
				String sql="  select m.part_no 'product id',m.productname 'product name',bd.brandname 'Brand Name',bc.branchname 'Branch',lc.loc_name 'Location',c.qty,m.jdeno,m.bhno,c.amount from "
						 + "  (select  b.locid,b.brhid,b.qty qty,b.prdid,amount amount from (select i.locid,i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
						 + "  ,i.cost_price*(i.op_qty-coalesce(a.outqty,0)) amount from my_prddin i "
						 + "  left join (select sum(qty)+sum(del_qty)+sum(rsv_qty) outqty, stockid ,prdid from my_prddout group by stockid) a "
						 + "  on a.stockid=i.stockid   where 1=1 group by i.stockid) b  ) c left join my_main m on m.doc_no=c.prdId "
      ...
      /*  
		...
		try {
				...
				*/
				...
				String sql="  select m.part_no 'product id',m.productname 'product name',bd.brandname 'Brand Name',m.barcode 'Bar Code',m.jdeno,m.bhno,ca.category 'Category',sc.subcategory 'Subcategory',bc.branchname 'Branch',lc.loc_name 'Location',c.qty,u.unit 'Unit' from "
						 + "  (select  b.locid,b.brhid,b.qty qty,b.prdid,amount amount from (select i.locid,i.brhid,i.op_qty-coalesce(a.outqty,0) qty,i.prdid "
						 + "  ,i.cost_price*(i.op_qty-coalesce(a.outqty,0)) amount from my_prddin i "
						 + "  left join (select sum(qty)+sum(del_qty)+sum(rsv_qty) outqty, stockid ,prdid from my_prddout group by stockid) a "
						 + "  on a.stockid=i.stockid   where 1=1 group by i.stockid) b  ) c left join my_main m on m.doc_no=c.prdId "
	...
	public JSONArray productSearch(HttpSession session,String docnos,String prdid,String prdname,String load) throws SQLException {
		...
		try {
			...
			String sql="select m.psrno doc_no,m.part_no prodcode,m.productname prodname,um.unit,m.jdeno,m.bhno from my_main m inner join my_brand b on(m.brandid=b.doc_no)"
