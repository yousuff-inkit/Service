package com.dashboard.sales.salesorderprocess;
...
public class ClsSalesorderprocessDAO { 
 ...
 /*
				...
				*/
				String sql="  select m.brhid branchvals,m.cldocno,m.doc_no,m.voc_no,a.refname client, "
						+ "  if(m.ref_type='DIR','Direct','Sales Quotation') category "
				+ "   from my_sorderm m left join my_acbook a on a.cldocno=m.cldocno and a.dtype='CRM' where    "
				+ "  m.DATE>='"+sqlfromdate+"' and   m.DATE<='"+sqltodate+"'  "+sqltest+" group by m.doc_no  order by m.voc_no ";
...
/*				String sql=" select bd.brandname,d.rdocno,d.prdId prodoc,m.part_no productid,m.productname,u.unit, d.unitid unitdocno,d.qty,"
						+ "  d.qty-d.outqty delqty, d.qty-(d.qty-d.outqty) belqty,aa.stkqty  "
						+ " from my_sorderm mm left join my_sorderd d on d.rdocno=mm.doc_no left join my_main m on m.doc_no=d.prdId left join  "
						...
						+ " left join(select sum(op_qty)-sum(out_qty+rsv_qty+del_qty) stkqty from my_prddin  "
						+ "  where 1=1 "+sqltest+"  and (op_qty)-(out_qty+rsv_qty+del_qty)>0 group by prdid) aa 	on aa.prdid=d.prdId  "
						+ "  where d.rdocno='"+docno+"'";
				*/
				
				String sql="   select brandname,rdocno,prodoc,productid,productname, "
						+ "    unit,unitdocno,qty,convert(if(delqty=0,'',delqty),char(100)) delqty,convert(if(belqty=0,'',belqty),char(100)) balqty, "
						+ "     convert(stkqty,char(100)) stock ,convert(if(po=0,'',po),char(100)) po,convert(if(belqty>(stkqty+po),(belqty-(stkqty+po)),''),char(100)) purchase  from "
						+ "    (select bd.brandname,d.rdocno,d.prdId prodoc,m.part_no productid,m.productname,u.unit, d.unitid unitdocno,d.qty, "
						+ "    d.out_qty delqty, d.qty-d.out_qty belqty,coalesce(aa.stkqty,'') stkqty,d.p_qty po   from my_sorderm mm left join my_sorderd d "
						+ "    on d.rdocno=mm.doc_no left join my_main m on m.doc_no=d.prdId left join   my_unitm u on d.unitid=u.doc_no "
						+ "    left join my_prodattrib at on(at.mpsrno=m.doc_no) left join  my_brand bd on m.brandid=bd.doc_no "
						+ "    left join(select sum(op_qty)-sum(out_qty+rsv_qty+del_qty) stkqty,prdid from my_prddin    where 1=1 "
						+ "    "+sqltest+" and (op_qty)-(out_qty+rsv_qty+del_qty)>0 group by prdid) aa 	on aa.prdid=d.prdId   where d.rdocno='"+docno+"' ) aa ";
	...
	public   JSONArray gridreloadsearch(String docno,String branch,String type,String clientid) throws SQLException {
		...
		try {
			...
			else
			{
				...
				String sql="  select * from (select   bd.brandname,d.refdocno,d.rdocno,d.prdId prodoc,m.part_no productid,m.productname,d.qty,d.stock,d.cost_price  costperunit, "
						...
						+ " 	(select  b.brandname,d.docno refdocno,d.rdocno,d.prdId prodoc,m.part_no productid, "
