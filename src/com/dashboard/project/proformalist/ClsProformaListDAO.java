package com.dashboard.project.proformalist;
...
public class ClsProformaListDAO {
	...
	public  JSONArray loadSummaryData(String fromdate,String todate,String clientid,String brch,String chk) throws SQLException {
		...
		String sqldata="select ROUND(@i:=@i+1 ,0) slno,a.* from(select s.sal_name salesperson,m.rate*sum(d.nettaxamount) nettotal,m.rate,c.code currency,b.branchname brhid,m.doc_no,m.voc_no,m.date,a.refname client,coalesce(m.invno,'') invno,m.desc1,sum(d.nettaxamount)total from my_srvsaleprom m "
				...
				+ "left join my_srvsaleprod d on m.doc_no=d.rdocno left join my_curr c on c.doc_no=m.curid where m.status<>7 "+sqltest1+" "+sqltest+" and m.date>='"+sqlfromdate+"' and m.date<='"+sqltodate+"' group by d.rdocno)a,(select @i:=0)b ";
		 
		//System.out.println("******+main6+******"+sqldata);
	...
	public  JSONArray loadDetailData(String fromdate,String todate,String clientid,String brch,String chk) throws SQLException {
		...
		String sqldata="select ROUND(@i:=@i+1 ,0) slno,a.* from(select m.date,m.rate*d.nettaxamount nettaxamountb,m.rate*d.total totalb,m.rate*d.nettotal nettotalb,m.rate*d.tax taxperamtb,m.rate,c.code currency,b.branchname brhid,m.voc_no,a.refname client,d.srno,d.desc1 description,d.unitprice,d.qty,d.qty qutval,d.total,d.discount,d.nettotal,d.nuprice,"
				...
				+ "d.taxper,d.tax taxperamt,d.nettaxamount taxamount  from my_srvsaleprod d left join my_srvsaleprom m on d.rdocno=m.doc_no left join my_brch b on b.doc_no=m.brhid left join my_head hd on hd.doc_no=m.acno left join my_head h on h.doc_no=d.acno  "
				+ "left join my_costunit u on u.costtype=d.costtype left join my_acbook a on a.acno=hd.doc_no and a.dtype='CRM'left join my_curr c on c.doc_no=m.curid where m.status<>7 and m.date>='"+sqlfromdate+"' and m.date<='"+sqltodate+"' "+sqltest+" "+sqltest1+")a,(select @i:=0)b";
		 
		//System.out.println("******+mainDetail7+******"+sqldata);
				resultSet= stmt.executeQuery (sqldata);
			    RESULTDATA=cm.convertToJSON(resultSet);
			//			System.out.println("=====RESULTDATA"+RESULTDATA);
