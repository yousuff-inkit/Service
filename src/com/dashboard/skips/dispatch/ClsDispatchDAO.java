package com.dashboard.skips.dispatch;   
...
//						System.out.println("--------underwriter----------"+sql2);
	...
	public JSONArray SummaryData(HttpSession session,int id,String fromdate,String todate) throws SQLException{
		...
		try {
			...
			*/   
		      sql="select sp.remarks,sp.ackdate,sp.date disdate,c.refno,m.dtype,case when m.statusid=1 then 'Hard copy submitted' when m.statusid=2 "
				       + "then 'Received acknowledgement' when m.email=1 then 'Email send' when m.statusid=3 then 'Printed' else "  
				       ...
				       + "coalesce(c.email,'') cntemail,sl.name dispatch,sp.auserid,coalesce(sp.receivedby,'') receivedby,"
				       + "coalesce(us.user_id,'') asuser,d.*  from "
					   + "sk_invm m  left join sk_srvcontrm c on c.doc_no=m.cnttrno left join my_acbook a on a.cldocno=c.cldocno and a.dtype='crm' left join my_salm sm on a.sal_id=sm.doc_no "
					   ...
					   + "left join sk_dispatchstatus sl on sl.rowno=m.dispatchid "
		               + "left join (select max(rowno) rno,rdocno,rdtype from sk_dispatchlog group by rdocno,rdtype) dis on dis.rdocno=m.doc_no and dis.rdtype=m.dtype "
		               + "left join sk_dispatchlog sp on sp.rowno=dis.rno left join (select min(rowno) rno,rdocno,rdtype from sk_dispatchlog group by rdocno,rdtype) d2 on d2.rdocno=m.doc_no and d2.rdtype=m.dtype left join sk_dispatchlog sp2 on sp2.rowno=d2.rno "
					   + "left join my_user us on sp.auserid=us.doc_no "
					   + " where  m.date between '"+sqlfromdate+"' and '"+sqltodate+"'  and m.status=3  group by invno";
