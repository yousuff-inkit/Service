...
import com.operations.marketing.enquirymarbles.ClsEnquiryMarblesDAO;
import com.operations.marketing.enquirymarbles.ClsEnquiryMarblesBean;

@SuppressWarnings("serial")
public class ClsEnquiryMarblesAction extends ActionSupport{
	ClsEnquiryMarblesDAO EnquiryDAO= new ClsEnquiryMarblesDAO(); 
	ClsCommon com=new ClsCommon();
	ClsEnquiryMarblesBean pintbean= new ClsEnquiryMarblesBean(); 
	private int docno;
	...
	private String txtclientname,txtsalesman,txtsalesmandocno;
	private int genaral;
	private int client;
	private int amc;
	private int sjob;
	...
	private int siteGridlength;
	  ...
	  public int getChksurvey() { return chksurvey; } public void setChksurvey(int
	  chksurvey) { this.chksurvey = chksurvey; } private int hidsurvey; public int
	  getHidsurvey() { return hidsurvey; }
	...
	public String saveAction() throws ParseException, SQLException{
        ...
        String Strsql="select sc.subcategory,c.category from gl_enqm e left join my_acbook ac on ac.cldocno=e.cldocno and ac.dtype='CRM' left join my_clcatm c on c.doc_no=ac.catid left join my_clsubcatm sc on sc.doc_no=ac.subcatid where e.doc_no="+getMasterdoc_no();
         System.out.println("Strsql========="+Strsql);
         ResultSet rs=stmt.executeQuery(Strsql);
         while (rs.next())       
         {
             setTxtcat(rs.getString("category"));
             setTxtsubcat(rs.getString("subcategory"));    
         }
		...
		if(mode.equalsIgnoreCase("A")){
			...
			int val=EnquiryDAO.insert(sqlStartDate,getCmbclient(),getTxtclientname(),getTxtaddress(),mobno,getGenaral(),getClient(),
					getTxtRemarks(),getTxtemail(),enqarray,session,getMode(),getFormdetailcode(),getTxttelno(),request,getSourceid(),getTxtcontact(),getCpersonid(),getCnt(),getCmbprocess(),sitearray,getChksurvey(),getTxtsalesmandocno(),getTxtsalesman(),getProname(),getHidtrno());
			...
			boolean Status=EnquiryDAO.edit(getMasterdoc_no(),sqlStartDate,getCmbclient(),getTxtclientname(),getTxtaddress(),mobno,getGenaral(),getClient(),getTxtRemarks(),
					getTxtemail(),enqarray,session,getMode(),getFormdetailcode(),getTxttelno(),getSourceid(),getTxtcontact(),getCpersonid(),getCnt(),getCmbprocess(),sitearray,getChksurvey(),getTxtsalesmandocno(),getTxtsalesman());
			if(Status){
			...
			boolean Status=EnquiryDAO.delete(getMasterdoc_no(),sqlStartDate,getCmbclient(),getTxtclientname(),getTxtaddress(),mobno,getGenaral(),getClient(),getTxtRemarks(),
					getTxtemail(),enqarray,session,getMode(),getFormdetailcode(),getTxttelno(),getSourceid(),getTxtcontact(),getCpersonid(),getCnt(),getTxtsalesmandocno(),getTxtsalesman());
			if(Status){
	...
	public  String searchTariff(){
		...
		try {
			List <ClsEnquiryMarblesBean> list= EnquiryDAO.list4();
			for(ClsEnquiryMarblesBean bean:list){
