...
public class ClspurchaseorderExcelImport {
	...
	public int ExcelImport(String docNo) throws SQLException {
		...
		try {
			...
			String path = "";
			String sql = "select path from my_fileattach where doc_no='" + docNo + "' and dtype='PO'";
			...
			for (int i = 1; i <= sheet.getLastRowNum(); i++) {
				...
				Double qty = formatter.formatCellValue(row.getCell(5)).trim() == "" ? 0: Double.parseDouble(formatter.formatCellValue(row.getCell(5)).trim());
				Double unitPrice = formatter.formatCellValue(row.getCell(6)).trim() == "" ? 0: Double.parseDouble(formatter.formatCellValue(row.getCell(6)).trim());
				Double discPerc = formatter.formatCellValue(row.getCell(8)).trim() == "" ? 0: Double.parseDouble(formatter.formatCellValue(row.getCell(8)).trim());
				Double discAmt = formatter.formatCellValue(row.getCell(9)).trim() == "" ? 0: Double.parseDouble(formatter.formatCellValue(row.getCell(9)).trim());

				Double total = qty*unitPrice;
				
				String sql2 = "INSERT INTO my_pod_excel (part_no, product_name, description, qty, unit_price, total, disc_perc, disc_amt, id) " 
				+ "VALUES ('" + partNo + "', '" + productName + "', '" + description + "', " + qty + ", " + unitPrice + ", " + total  + ", " + discPerc + ", " + discAmt + ", " + docNo + ");";
