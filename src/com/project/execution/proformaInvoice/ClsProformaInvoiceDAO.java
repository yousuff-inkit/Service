...
public int insert(Date sqlStartDate,Date purdeldate, String reftype,String refno, String acctype,
		...
		HttpServletRequest request, Date sqlinvdate, String invno,String indateval,int interstate,Double taxperc,ClsProformaInvoiceAction masteraction) throws SQLException {
	try{
		int docno;
...
//		 System.out.println("cmbcurr---"+cmbcurr+"=currate="+currate);
 ...
 if(docval==0)
 {
		...
		CallableStatement stmtnipurchase= conn.prepareCall("{call ServiceSaleProDML(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
...
//		System.out.println("--stmtnmipurchase-- "+stmtnipurchase.toString());
		...
		if(docno<=0)
		{
			...
			String strupdatemaster="update my_srvsaleprom set delno='"+masteraction.getDelno()+"' where doc_no="+docno+" and brhid="+session.getAttribute("BRANCHID").toString()+"";
		    ...
		    if(!(purorderarray[1].trim().equalsIgnoreCase("undefined")|| purorderarray[1].trim().equalsIgnoreCase("NaN")||purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()))
		     {
	    		 ...
	    		 String sql="INSERT INTO my_srvsaleprod(srno,qty,desc1,unitprice,total,discount,nettotal,taxper,tax,nettaxamount,nuprice,costtype,costcode,remarks,acno,brhid,rdocno)VALUES"
					       + " ("+(i+1)+","
			...
			int samp=stmtnipurchase.executeUpdate(don);
			 ...
			 if(!(purorderarray[11].trim().equalsIgnoreCase("undefined")|| purorderarray[11].trim().equalsIgnoreCase("NaN")||purorderarray[11].trim().equalsIgnoreCase("")|| purorderarray[11].isEmpty() ||purorderarray[11].trim().equalsIgnoreCase("0")))
		     {
			  ...
			  if(costabsq<=0)
				...
				String updat="update  my_srvsaleprom set invno='"+invno+"',invdate='"+sqlinvdate+"' where doc_no="+docno+"  ";
		...
		/* ArrayList<String> arr=new ArrayList<String>(); 
			...
			String selectsqls= "select sum(a.nettaxamount) nettaxamount,sum(a.total1) total1,sum(a.total2) total2,sum(a.total3) total3, "
				...
				+" 	 select  d.nettotal+coalesce(d.tax,0) nettaxamount,if(coalesce(d.tax,0)>0,d.nettotal,0) total1, "
					...
					+"  0 tax7,  0 tax8, "
					+"  0 tax9,  0 tax10 "
					 +"  from my_srvsaleprod d where rdocno="+docno+" ) a" ;
...
public boolean edit(int docno,Date sqlStartDate,Date purdeldate, String reftype, String refno, String acctype,
		...
		String invno,String indateval,int interstate,Double taxperc,ClsProformaInvoiceAction masteraction) throws SQLException {
...
//		System.out.println("=="+stmtnipurchase.toString());
	    ...
	    double cmbcurrency=1.0,rate=1.0;   
		   ...
		   String delsql="Delete from my_srvsaleprod where rdocno="+docno+" and brhid='"+session.getAttribute("BRANCHID").toString()+"' ";
		  ...
		  /* String delsql1="Delete from my_jvtran where tr_no="+tranno+" ";
		   ...
		   stmtnipurchase.executeUpdate(delsql3);*/
			...
			String sqlss="select voc_no from my_srvsaleprom where doc_no='"+docno+"' ";
			...
			String refdetails="PINV"+""+vocno;
			//String jvdesc="PINV-"+""+invno+""+":-Dated :- "+indateval+"";  
			String jvdesc=""; 
	...
	//	System.out.println("aaaaaaaaaaaaaaaaa"+aaa);
		/*for(int i=0;i<descarray.size();i++){
		     ...
		     if(!(purorderarray[1].trim().equalsIgnoreCase("undefined")|| purorderarray[1].trim().equalsIgnoreCase("NaN")||purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()))
		     {
		    	 ...
		    	 String sql="INSERT INTO my_srvsaleprod(srno,qty,desc1,unitprice,total,discount,nettotal,nuprice,costtype,costcode,remarks,acno,brhid,rdocno)VALUES"
					       + " ("+(i+1)+","
					       + "'"+(purorderarray[1].trim().equalsIgnoreCase("undefined") || purorderarray[1].trim().equalsIgnoreCase("NaN")|| purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()?0:purorderarray[1].trim())+"',"
					       + "'"+(purorderarray[2].trim().equalsIgnoreCase("undefined") || purorderarray[2].trim().equalsIgnoreCase("NaN")|| purorderarray[2].trim().equalsIgnoreCase("")|| purorderarray[2].isEmpty()?0:purorderarray[2].trim())+"',"
					       + "'"+(purorderarray[3].trim().equalsIgnoreCase("undefined") || purorderarray[3].trim().equalsIgnoreCase("NaN")||purorderarray[3].trim().equalsIgnoreCase("")|| purorderarray[3].isEmpty()?0:purorderarray[3].trim())+"',"
		...
		/*    	 
	     String sql="update  my_srvsaleprod set SRNO="+(i+1)+","
...
//		System.out.println("0000000000000000-0000000000000");
		...
		ClsVatInsert ClsVatInsert=new ClsVatInsert();
		Statement newStatement=conn.createStatement();
		String selectsqls= " select tr_no from my_srvsaleprom where doc_no="+docno+" " ;
...
public   JSONArray reloadnipurchase(HttpSession session,String nidoc) throws SQLException {
		...
		try {
	        	...
	        	String pySql=(" select case when d.costtype=1 then cc.doc_no when d.costtype in(3,4) then cm.doc_no when d.costtype=11 then sc.voc_no when d.costtype=13 then ac.voc_no else 0 end as costcodeid,d.srno,d.desc1 description,d.unitprice,d.qty,d.qty qutval,d.total,d.discount,d.nettotal,d.nuprice,d.acno headdoc,h.gr_type grtype ,"
	        			...
	        			+ " from my_srvsaleprom m inner join my_srvsaleprod d on d.rdocno=m.doc_no left join my_head h on h.doc_no=d.acno  left join my_costunit u on u.costtype=d.costtype left join cm_srvcontrm cm on cm.doc_no=d.costcode and cm.dtype=if(d.costtype=3,'AMC',if(d.costtype=4,'SJOB','')) and cm.brhid=m.brhid "
  ...
  try {
   ...
   e.printStackTrace();
		...
		try {
			...
			if(aa.equalsIgnoreCase("yes"))
			{
	        	...
	        	String pySql=("select coalesce(m.delno,'') delno,a.per_mob mobno,convert(coalesce(m.refno,''),char(20)) refvocno,m.tr_no,m.doc_no,m.voc_no,coalesce(m.invno,'') invno,m.invdate,m.date,m.netamount,m.type,m.acno,m.reftype,m.refno,m.curid,m.rate,m.delterm,m.payterm, "
	        			+ "m.deldate,m.desc1,h.description,h.account,m.interstate,h.atype from my_srvsaleprom m left join my_head h on h.doc_no=m.acno left join my_srvlpom o on m.refno=o.doc_no left join my_acbook a on a.acno=h.doc_no and a.dtype='CRM' where m.status<>7  "
	...
	public  ClsProformaInvoiceBean getPrint(int docno, HttpServletRequest request ,HttpSession session) throws SQLException {
		...
		try {
				...
				/*String resql=("select m.invno,date_format(m.invdate,'%d-%m-%Y') invdate,m.doc_no,m.voc_no,date_format(m.date,'%d-%m-%Y') date,round(m.netamount,2) netamount,m.type,m.acno,if(m.reftype='DIR','Direct',concat('NI Purchase Order ','(',m.refno,')')) reftype,m.refno,m.curid,m.rate,m.delterm, "
						+ "coalesce(m.payterm,'') payterm,date_format(m.deldate,'%d-%m-%Y') deldate,coalesce(m.desc1,'') desc1,coalesce(h.description,'') description,h.account from my_srvsaleprom m left join my_head h on h.doc_no=m.acno  where m.DOC_NO='"+docno+"' ");
				*/
				String resql = ("select m.curid,coalesce(m.delno,'') delno,if(m.date<20210101,0,1) easycmp,ar.area,cr.country_name as country,year(curdate())curyear,m.voc_no,if(CONCAT(if(a.per_tel is null or trim(a.per_tel)='',if(a.per_mob is null or trim(a.per_mob)='',if(a.com_mob is null or trim(a.com_mob)='',' NIL , ',CONCAT(a.com_mob,' , ')),CONCAT(a.per_mob,' , ')),CONCAT(a.per_tel,' , ')), "
					...
					+ "h.account,round(sum(d.tax),2) taxamount from my_srvsaleprom m left join my_head h on h.doc_no=m.acno left join my_acbook a on a.acno=h.doc_no and a.dtype='CRM' left join my_area ar on a.area_id=ar.doc_no left join my_acountry cr on ar.country_id=cr.doc_no "
					+ "left join my_srvsaleprod d on m.doc_no=d.rdocno  left join my_brch b on m.brhid=b.doc_no"
...
//				System.out.println("branchhhkkkkk"+session.getAttribute("BRANCHID").toString().trim());
				...
				stmtinvoice11.close();
				 ...
				 /*   String  companysql="select b.branchname,c.company,c.address,c.tel,c.fax,l.loc_name location from my_srvsaleprom r  "
				    		+ " left join my_brch b on r.brhid=b.doc_no left join my_locm l on l.brhid=b.doc_no "
				    		+ "left join my_comp c on b.cmpid=c.doc_no where r.doc_no="+docno+"  ";*/
				    String companysql = "select c.company,c.address,c.tel,c.fax,lc.loc_name location,b.branchname,b.pbno,b.stcno,b.cstno,b.tinno from my_srvsaleprom r inner join my_brch b on  "
				...
				bean.setLblbranchtrno(resultsetcompany.getString("tinno"));
				 ...
				 ArrayList<String> arr=new ArrayList<String>();
						...
						String strSqldetail="select d.srno,d.desc1 description,format(d.unitprice,2) unitprice,round(d.qty) qty,format(d.total,2) total,coalesce(format(d.discount,2),'')discount,format(d.nettotal,2) nettotal,d.nuprice,"
								+ " d.remarks,h.account account,h.description accname,h.atype accounttype,round(d.taxper,2) taxper,format(d.tax,2) taxperamt,format(d.nettaxamount,2) nettaxamount  from my_srvsaleprod d left join my_head h on h.doc_no=d.acno  "
					...
					String branchsql="select  b.branchname,b.address,b.tel,b.fax,l.loc_name location,coalesce(b.tinno,0) tinno from my_srvsaleprom m left join my_brch b on m.brhid=b.doc_no left join my_locm l on l.brhid=b.doc_no where m.doc_no="+docno;
...
//					System.out.println("rst"+username);
			...
			/*		String crsql="select coalesce(m.invno,'') jobno,coalesce(cm.refno,'') cusref,ac.refname,coalesce(ac.address,'') address, "
							...
							+ " left join my_acbook ac on ac.acno=m.acno  and ac.dtype='crm' left join my_srvsaleprod d on m.doc_no=d.rdocno"
					...
					String crsql=" select qm.confirm_no jobno,coalesce(m.invno,'') subjobno,coalesce(cm.refno,'') cusref,ac.refname,coalesce(ac.address,'') address,"
							...
							+ "coalesce(ed.noofpacks,'') qty  from my_srvsaleprom m   left join(select invtrno,rdocno  from  cr_cfid where invtrno>0 "
							...
							+ "wordsamount,rdocno from my_srvsaleprod d group by d.rdocno) dd2 on dd2.rdocno=m.doc_no   "
		...
		public   JSONArray searchCostcodes(String type) throws SQLException {  
			...
			try {
		        	...
		        	else if(type.equalsIgnoreCase("3") || type.equalsIgnoreCase("4"))
		        	{
		        		...
		        		String sql=" select m.doc_no,cast(concat(m.doc_no, if(sd.doc_no is null,'',concat('',sd.doc_no))) as char(20)) code,concat(if(sd.name is null,'' ,concat(sd.name,'')),a.refname) name from cm_srvcontrm m left join my_acbook a on m.cldocno=a.cldocno and a.dtype='CRM' "
