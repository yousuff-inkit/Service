...
import com.project.execution.ServiceSale.ClsServiceSaleBean;
	...
	public class ClsServiceSaleAction3d extends ActionSupport{

		ClsCommon commDAO=new ClsCommon();

		ClsServiceSaleDAO3d purchaseDAO= new ClsServiceSaleDAO3d(); 
		ClsServiceSaleBean3d pintbean= new ClsServiceSaleBean3d(); 
		...
		private String refno,acctype,nipuraccid,puraccname,cmbcurr,hidcmbcurr,currate,deliverydate,hiddeliverydate,delterms,payterms,purdesc,mode,msg, cmbcurrval,acctypeval,accdocno,formdetailcode,appno,delnote,appdate;
	...
	public void setLblbankibanno(String lblbankibanno) {
		...
		public String saveAction() throws ParseException, SQLException{
			...
			java.sql.Date sqlappdate = commDAO.changeStringtoSqlDate(getAppdate());
			//System.out.println("date==="+getAppdate());
	...
	if(mode.equalsIgnoreCase("A")){
		...
		int val=purchaseDAO.insert(sqlStartDate1,sqlpurdeldate,sqlappdate,getNireftype(),getRefno(),getAcctype(),getAccdocno(),getPuraccname(), 
				getCmbcurr(),getCurrate(),getDelterms(),getPayterms(),getPurdesc(),session,getMode(),getNettotal(),descarray,getFormdetailcode(),
				request,sqlinvdate,getInvno(),getInvDate(),getInterstate(),getTaxperc(),getAppno(),getDelnote());
		...
		if(val>0){
			...
			setAppdate(sqlappdate.toString());
			...
			setAppno(getAppno());
			setDelnote(getDelnote());
			pintbean=(ClsServiceSaleBean3d)request.getAttribute("pintbean");   
			if(pintbean!=null){
			...
			boolean Status=purchaseDAO.edit(getMasterdoc_no(),sqlStartDate1,sqlpurdeldate,sqlappdate,getNireftype(),getRefno(),
					getAcctype(),getAccdocno(),getPuraccname(), getCmbcurr(),getCurrate(),getDelterms(),getPayterms(),
					getPurdesc(),session,getMode(),getNettotal(),descarray,getFormdetailcode(),getTarannumber(),request,sqlinvdate,getInvno(),getInvDate(),getInterstate(),getTaxperc(),getAppno(),getDelnote());
			if(Status){
			...
			boolean Status=purchaseDAO.delete(getMasterdoc_no(),session,getMode(),getFormdetailcode(),sqlappdate);
		if(Status){
			setOrdermasterdoc_no(getOrdermasterdoc_no()) ;
			setRefno(getRefno());
			setReftypeval(getNireftype());
		 ...
		 public String printAction() throws ParseException, SQLException{
			 ...
			 System.out.println("dtype"+dtype);
			  ...
			  //cl details
			    	...
			    	setDelnote(pintbean.getDelnote());
			    	setAppno(pintbean.getAppno());
			    	setAppdate(pintbean.getAppdate());
...
//			          System.out.println("adrsss+++++++++"+bean.getLblvenaddress());
			          ...
			          param.put("delnote", bean.getDelnote());
			          param.put("appno", bean.getAppno());
			          param.put("appdate", bean.getAppdate());
			          //System.out.println("app date==="+bean.getAppdate());
...
//			          System.out.println("headerstatus"+header);
	             ...
	             JasperDesign design = JRXmlLoader.load(request.getSession().getServletContext().getRealPath("com/project/execution/ServiceSale3dcom/" +commDAO.getPrintPath(dtype)));
