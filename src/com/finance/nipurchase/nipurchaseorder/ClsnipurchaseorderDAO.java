...
public class ClsnipurchaseorderDAO  {
	...
	public int insert(Date sqlStartDate,Date purdeldate, String refno, String acctype,
			...
			HttpSession session, String mode,Double nettotal,ArrayList<String> descarray,String Formdetailcode,HttpServletRequest request,int ptype,String reftype,String reqrefno) throws SQLException {
		...
		try{
			...
			if(yrconfig==5) {
				...
				stmtpurorder.setString(18, objvocapp);
			
			}else {
			
				stmtpurorder.registerOutParameter(18, java.sql.Types.INTEGER);
					
			}
			//System.out.println("objvocapp-----"+objvocapp);
			stmtpurorder.registerOutParameter(17, java.sql.Types.INTEGER);
			...
			stmtpurorder.setString(12,reftype);
			stmtpurorder.setString(13,reqrefno);
			stmtpurorder.setString(14,Formdetailcode);
			stmtpurorder.setString(15,session.getAttribute("USERID").toString());
			stmtpurorder.setString(16,session.getAttribute("BRANCHID").toString());

			stmtpurorder.setString(19,mode);
			...
			for(int i=0;i< descarray.size();i++){
		     ...
		     String sql="INSERT INTO my_srvlpod(srno,qty,desc1,unitprice,total,discount,nettotal,nuprice,taxper,taxamount,nettaxamount,refrow,brhid,rdocno)VALUES"
				       + " ("+(i+1)+","
				     ...
				     String reqsql = "update my_srvreqd set out_qty=out_qty+"+(purorderarray[1].trim().equalsIgnoreCase("undefined") || purorderarray[1].trim().equalsIgnoreCase("NaN")|| purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()?0:purorderarray[1].trim())
				    		 +" where rowno="+(purorderarray[11].trim().equalsIgnoreCase("undefined") || purorderarray[11].trim().equalsIgnoreCase("NaN")|| typedocval==0||purorderarray[11].trim().equalsIgnoreCase("")|| purorderarray[11].isEmpty()?0:purorderarray[11].trim());
				     stmtpurorder.executeUpdate(reqsql);
	...
	public boolean edit(int docno,Date sqlStartDate,Date purdeldate, String refno, String acctype,
			String accdoc, String puraccname, String cmbcurr, String currate,
			String delterms, String payterms, String purdesc,
			HttpSession session, String mode,Double nettotal,ArrayList<String> descarray,String Formdetailcode,int ptype,String reftype,String reqrefno) throws SQLException {
		...
		try{
			...
			stmtpurorder.setString(15,session.getAttribute("USERID").toString());
			stmtpurorder.setString(16,session.getAttribute("BRANCHID").toString());
			stmtpurorder.setInt(17,docno);
			stmtpurorder.setInt(18,0);
			stmtpurorder.setString(19,"E");
			...
			String dsql="update my_srvreqd rd inner join my_srvlpod d on rd.rowno=d.refrow set rd.out_qty=rd.out_qty-d.qty where d.rdocno="+docno; 
			stmtpurorder.executeUpdate(dsql);
	...
	public boolean delete(int docno,HttpSession session,String mode,String Formdetailcode) throws SQLException {
		...
		//	conn.setAutoCommit(false);
			...
			stmtpurorder.setString(12,null);
			stmtpurorder.setString(13,null);
			stmtpurorder.setString(14,Formdetailcode);
			stmtpurorder.setString(15,session.getAttribute("USERID").toString());
			stmtpurorder.setString(16,session.getAttribute("BRANCHID").toString());
			stmtpurorder.setInt(17,docno);
			stmtpurorder.setInt(18,0);
			stmtpurorder.setString(19,"D");
   ...
   conn.close();
	...
	//    System.out.println("==========brcid======"+brcid);
		...
		try {
	        	...
	        	String pySql=("select d.srno,d.desc1 description,d.unitprice,d.qty,d.total,d.discount,d.nettotal,d.nuprice,d.taxper,d.taxamount taxperamt,d.nettaxamount taxamount,d.refrow ,coalesce(((rd.qty-rd.out_qty)+d.qty),0) avlqty "
	        			+"from my_srvlpod d left join my_srvreqd rd on rd.rowno=d.refrow where d.rdocno='"+nidoc+"'");
//	        	//System.out.println("====---===="+pySql);
	...
	public JSONArray loadnireq(HttpSession session,String nidoc,String reqrowno) throws SQLException {
	  ...
	  return RESULTDATA;
		...
		try {
				...
				if((nidoc.equalsIgnoreCase("undefined"))||(nidoc.equalsIgnoreCase(""))||(nidoc.equalsIgnoreCase("0"))){
					nidoc="0";
				}
				
	        	String pySql=("select d.srno,d.desc1 description, (d.qty-d.out_qty)+coalesce(od.qty,0) qty, (d.qty-d.out_qty)+coalesce(od.qty,0) avlqty, d.rowno refrow from my_srvreqd d left join my_srvlpod od on d.rowno=od.refrow and od.rdocno="+nidoc+" where ((d.qty-d.out_qty)+coalesce(od.qty,0))>0 and d.rowno in ("+reqrowno+")");
...
/*        String branch = session.getAttribute("BRANCHID").toString();
 ...
 /*  String sql=("select (@i:=@i+1) recno,t.doc_no,t.account,t.description,c.code curr from my_head t,my_acbook a1, "
   ...
   conn.close();
	...
	public JSONArray niPurchaseRequestSearch(HttpSession session,String msdocno,String nipureqdate,String requestedby, String remark, String desc, String orderdocno, int id) throws SQLException {
		...
		java.sql.Date sqlStartDate=null;
		String str1Sql="";
		if(!(nipureqdate.equalsIgnoreCase("undefined"))&&!(nipureqdate.equalsIgnoreCase(""))&&!(nipureqdate.equalsIgnoreCase("0")))
		{
			sqlStartDate = commDAO.changeStringtoSqlDate(nipureqdate);
			sqltest=sqltest+" and m.date<="+sqlStartDate+"";
		}
		...
		if(!(msdocno.equalsIgnoreCase("undefined"))&&!(msdocno.equalsIgnoreCase(""))&&!(msdocno.equalsIgnoreCase("0"))){
	 	...
	 	if((!(requestedby.equalsIgnoreCase(""))) && (!(requestedby.equalsIgnoreCase("NA")))){
      		sqltest=sqltest+" and m.requestedby like '%"+requestedby+"%'";
      	}
    	
      	if((!(remark.equalsIgnoreCase(""))) && (!(remark.equalsIgnoreCase("NA")))){
      		sqltest=sqltest+" and m.desc1 like '%"+remark+"%'";
      	}
		
      	if((!(desc.equalsIgnoreCase(""))) && (!(desc.equalsIgnoreCase("NA")))){
      		sqltest=sqltest+" and d.desc1 like '%"+desc+"%'";
      	}
      	
		if((orderdocno.equalsIgnoreCase("undefined"))||(orderdocno.equalsIgnoreCase(""))||(orderdocno.equalsIgnoreCase("0"))){
			orderdocno="0";
		}
		...
		try {
			...
			if(id>0){
				str1Sql=("select m.doc_no, m.voc_no, m.date, m.requestedby, m.desc1 remark, d.rowno, d.desc1 description, (d.qty-d.out_qty)+coalesce(od.qty,0) qty from my_srvreqm m inner join my_srvreqd d on d.rdocno=m.doc_no left join my_srvlpod od on d.rowno=od.refrow and od.rdocno="+orderdocno+" where m.status<>7 and ((d.qty-d.out_qty)+coalesce(od.qty,0))>0 and m.brhid='"+brcid+"' "+sqltest+" order by m.doc_no,d.srno" );
	...
	public ClsnipurchaseorderBean getPrint(int docno, HttpServletRequest request,HttpSession session,String brhid) throws SQLException {
		...
		try {
				...
				String resql=("select  coalesce(br.tinno,'') companytrno,coalesce(u.user_name,'') preparedby,format(round(aa.netamt,2),2) netamt,round(aa.netamt,2) wnetamt, m.status,u1.user_name verifiedappr,"
						...
						+ " m.refno,cr.code as curr,coalesce(a.mail1,'') mail1,m.refno,  coalesce(a.fax1,'') fax1,coalesce(a.per_mob,a.com_mob) per_mob,"
						...
						+ " and ext1.apprstatus=3) left join my_user u on m.userid=u.doc_no left join my_user u1 on ext.userid=u1.doc_no left join my_curr cr on(m.curid=cr.doc_no)"
...
*/  String  companysql="select c.company,c.address,c.tel,c.fax,coalesce(lc.loc_name,'') location,coalesce(b.branchname,'') branchname,b.pbno,b.stcno,b.cstno,b.tinno from my_srvlpom r inner join my_brch b on  "
	...
	public ClsnipurchaseorderBean getViewDetails(int docno, HttpSession session) throws SQLException {
    	...
    	String pySql=("select  coalesce(m.typeid,0) typeid,coalesce(p.ptype,'') ptype,coalesce(p.per,0) per ,m.doc_no,m.voc_no,m.date,m.netamount,m.type,m.acno,m.refno,m.curid,m.rate,m.delterm,m.payterm,m.deldate,m.desc1,h.description,h.account,m.reftype,m.reqrefno "
		...
		showBean.setAcctypeval(resultSet.getString("type"));
		 //showBean.setCurrency(resultSet.getString("curr"));
		...
		showBean.setHidcmbreftype(resultSet.getString("reftype"));
		showBean.setTxtreqrefno(resultSet.getString("reqrefno"));
